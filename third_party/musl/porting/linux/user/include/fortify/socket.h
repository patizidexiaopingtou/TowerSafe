...
ssize_t __sendto_diagnose(int, const void*, size_t, size_t, int, const struct sockaddr*,
    socklen_t);
ssize_t __recvfrom_diagnose(int, void*, size_t, size_t, int, struct sockaddr*,
    socklen_t*);
ssize_t __send_diagnose(int, const void*, size_t, size_t, int);
ssize_t __recv_diagnose(int, void*, size_t, size_t, int);
    ...
    "'recvfrom' " CALLED_WITH_SIZE_BIGGER_BUFFER)
{
    ...
    if (!__DIAGNOSE_BOS_TRIVIALLY_GE(bos, len)) {
        return __recvfrom_diagnose(fd, buf, len, bos, flags, src_addr, addr_len);
    }
#endif
    ...
    "'sendto' " CALLED_WITH_SIZE_BIGGER_BUFFER)
{
    ...
    if (!__DIAGNOSE_BOS_TRIVIALLY_GE(bos, len)) {
        return __sendto_diagnose(fd, buf, len, bos, flags, dest_addr, addr_len);
    }
#endif
    ...
    "'recv' " CALLED_WITH_SIZE_BIGGER_BUFFER)
{
    ...
    if (!__DIAGNOSE_BOS_TRIVIALLY_GE(bos, len)) {
        return __recv_diagnose(socket, buf, len, bos, flags);
    }
#endif
    ...
    "'send' " CALLED_WITH_SIZE_BIGGER_BUFFER)
{
    ...
    if (!__DIAGNOSE_BOS_TRIVIALLY_GE(bos, len)) {
        return __send_diagnose(socket, buf, len, bos, flags);
    }
#endif
