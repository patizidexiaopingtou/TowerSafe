...
void PagedMemoryDataParser::ParsePagedMemoryEvent()
{
    ...
    for (auto mapItor = reader_->GetPagedMemoryMap().begin();
         mapItor != reader_->GetPagedMemoryMap().end(); mapItor++) {
        ...
        if (!callIdExistFlag) {
            ParseCallStackData(userIpsAddr, pagedMemoryFixedHeadrAddr->nips,
                               pagedMemoryFixedHeadrAddr->pid, currentCallId_);
        }
    }
}
} // namespace TraceStreamer
