...
#if defined(BTA_AG_MULTI_RESULT_INCLUDED) && (BTA_AG_MULTI_RESULT_INCLUDED == 1)
...
#if defined(BTA_HSP_RESULT_REPLACE_COLON) && (BTA_HSP_RESULT_REPLACE_COLON == 1)
    ...
    if(p_scb->conn_service == BTA_AG_HSP) {
        ...
        switch(code) {
            ...
            case BTA_AG_RES_VGM:
            {
                ...
                {
                    #if defined(BTA_AG_RESULT_DEBUG) && (BTA_AG_RESULT_DEBUG == 1)
...
#endif /* #if defined(BTA_AG_MULTI_RESULT_INCLUDED) && (BTA_AG_MULTI_RESULT_INCLUDED == 1) */
...
**
**
** Returns          1 if parsed ok, 0 otherwise.
**
*******************************************************************************/
...
static tBTA_AG_PEER_CODEC bta_ag_parse_bac(tBTA_AG_SCB *p_scb, char *p_s)
{
    ...
    UINT16  uuid_codec;
    BOOLEAN cont = 0;       /* Continue processing */
    char *p;
...
void bta_ag_send_call_inds(tBTA_AG_SCB *p_scb, tBTA_AG_RES result)
{
    ...
    bta_ag_send_ind(p_scb, BTA_AG_IND_CALL, call, 0);
    bta_ag_send_ind(p_scb, BTA_AG_IND_CALLSETUP, callsetup, 0);
}
...
void bta_ag_hfp_result(tBTA_AG_SCB *p_scb, tBTA_AG_API_RESULT *p_result)
{
    ...
    switch(p_result->result)
    {
        ...
        case BTA_AG_IND_RES:
            bta_ag_send_ind(p_scb, p_result->data.ind.type, p_result->data.ind.value, 0);
            break;
