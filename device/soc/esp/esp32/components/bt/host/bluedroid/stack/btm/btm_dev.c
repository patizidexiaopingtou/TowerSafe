...
#if defined(BTIF_MIXED_MODE_INCLUDED) && (BTIF_MIXED_MODE_INCLUDED == 1)
...
**
** Returns          1 if removed OK, 0 if not found or ACL link is active
**
*******************************************************************************/
...
**
** Returns          1 if device is known and role switch is supported
**
*******************************************************************************/
BOOLEAN btm_dev_support_switch (BD_ADDR bd_addr)
{
    ...
    if (p_dev_rec && controller_get_interface()->supports_master_slave_role_switch()) {
        if (HCI_SWITCH_SUPPORTED(p_dev_rec->features[HCI_EXT_FEATURES_PAGE_0])) {
            BTM_TRACE_DEBUG("btm_dev_support_switch return 1 (feature found)\n");
            return (1);
        }
        ...
        for (xx = 0 ; xx < BD_FEATURES_LEN ; xx++) {
            if (p_dev_rec->features[HCI_EXT_FEATURES_PAGE_0][xx] != 0x00) {
                feature_empty = 0; /* at least one is != 0 */
                break;
            }
        }
        ...
        if (feature_empty) {
            BTM_TRACE_DEBUG("btm_dev_support_switch return 1 (feature empty)\n");
            return (1);
        }
    }

    BTM_TRACE_DEBUG("btm_dev_support_switch return 0\n");
    return (0);
}
