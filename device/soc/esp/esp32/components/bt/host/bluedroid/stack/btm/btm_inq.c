...
#if (defined(SDP_INCLUDED) && SDP_INCLUDED == 1)
#include "stack/sdpdefs.h"
#endif

#define BTM_INQ_REPLY_TIMEOUT   3       /* 3 second timeout waiting for responses */

/* 1 to enable DEBUG traces for btm_inq */
#ifndef BTM_INQ_DEBUG
#define BTM_INQ_DEBUG   0
#endif
/********************************************************************************/
...
tBTM_STATUS BTM_SetDiscoverability (UINT16 inq_mode, UINT16 window, UINT16 interval)
{
    ...
    is_limited = (inq_mode & BTM_LIMITED_DISCOVERABLE) ? 1 : 0;
    cod_limited = (service_class & BTM_COD_SERVICE_LMTD_DISCOVER) ? 1 : 0;
                                        ...
                                        UINT16 min_delay, tBTM_INQ_RESULTS_CB *p_results_cb)
{
    ...
    /* If illegal parameters return 0 */
...
#if (BLE_INCLUDED==1 && (defined(BTA_HOST_INTERLEAVE_SEARCH) && BTA_HOST_INTERLEAVE_SEARCH == 1))
...
#if (!defined(BTA_HOST_INTERLEAVE_SEARCH) || BTA_HOST_INTERLEAVE_SEARCH == 0)
...
**
** Returns          1 if found, else 0 (new entry)
**
*******************************************************************************/
static void btm_clr_inq_result_flt (void)
{
        ...
        tBTM_INQ_FILT_COND *p_filt_cond)
{
    ...
    btm_cb.btm_inq_vars.inqfilt_active = 1;
    ...
    for (xx = 0; xx < num_resp; xx++) {
        ...
        */
        ...
        if (is_new == 1) {
            ...
            /* Initialize flag to 0. This flag is set/used by application */
            p_i->inq_info.appl_knows_rem_name = 0;
        }
...
void btm_set_eir_uuid( UINT8 *p_eir, tBTM_INQ_RESULTS *p_results )
{
    ...
    if (type == BTM_EIR_COMPLETE_16BITS_UUID_TYPE) {
        p_results->eir_complete_list = 1;
    } else {
        p_results->eir_complete_list = 0;
    }
