...
#if (defined BTA_HH_INCLUDED && BTA_HH_INCLUDED == 1)
...
#endif /* (defined BTA_HH_INCLUDED && BTA_HH_INCLUDED == 1) */
...
static void bta_dm_pm_cback(tBTA_SYS_CONN_STATUS status, UINT8 id, UINT8 app_id, BD_ADDR peer_addr)
{
    ...
    for (j = 0; j < bta_dm_conn_srvcs.count ; j++) {
        ...
        if ((bta_dm_conn_srvcs.conn_srvc[j].id == id)
            ...
            bta_dm_conn_srvcs.conn_srvc[j].new_request = 1;
            break;
        }
...
**
**
** Returns          1 if park attempted, 0 otherwise.
**
*******************************************************************************/
static BOOLEAN bta_dm_pm_park(BD_ADDR peer_addr)
{
...
**
**
** Returns          1 if sniff attempted, 0 otherwise.
**
*******************************************************************************/
static BOOLEAN bta_dm_pm_sniff(tBTA_DM_PEER_DEVICE *p_peer_dev, UINT8 index)
{
...
void bta_dm_pm_btm_status(tBTA_DM_MSG *p_data)
{
    ...
    if ( bta_dm_cb.p_sec_cback ) {
        if (p_data->pm_status.status <= BTM_PM_STS_PARK
            /*&& p_data->pm_status.status >= BTM_PM_STS_ACTIVE*/  // comparison is always 1 due to limited range of data type
            ) {
            tBTA_DM_SEC conn;
...
**
** Returns          BOOLEAN. 1 if SCO active, else 0
**
*******************************************************************************/
static BOOLEAN bta_dm_pm_is_sco_active (void)
{
    int j;
    BOOLEAN bScoActive = 0;
...
#if ((defined BLE_INCLUDED) && (BLE_INCLUDED == 1) && SDP_INCLUDED == 1)
/*******************************************************************************
**
