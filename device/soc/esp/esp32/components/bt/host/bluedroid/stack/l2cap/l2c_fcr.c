...
void l2c_fcr_proc_pdu (tL2C_CCB *p_ccb, BT_HDR *p_buf)
{
    ...
    if (ctrl_word & L2CAP_FCR_S_FRAME_BIT) {
        ...
        process_i_frame (p_ccb, p_buf, ctrl_word, 0);
    }
    ...
    if ( (!p_ccb->fcrb.local_busy) && (!p_ccb->fcrb.srej_sent) &&
        ...
        while ((p_buf = (BT_HDR *)fixed_queue_dequeue(temp_q, 0)) != NULL) {
            if (p_ccb->in_use && (p_ccb->chnl_state == CST_OPEN)) {
                ...
                process_i_frame (p_ccb, p_buf, ctrl_word, 1);
            } else {
                osi_free (p_buf);
            }
            ...
            if (p_ccb->fcrb.rej_after_srej) {
                p_ccb->fcrb.rej_after_srej = 0;
                p_ccb->fcrb.rej_sent       = 1;
    ...
    if ( (!fixed_queue_is_empty(p_ccb->fcrb.retrans_q) ||
          !fixed_queue_is_empty(p_ccb->xmit_hold_q))
            && (p_ccb->fcrb.wait_ack == 0)
            && (l2c_fcr_is_flow_controlled (p_ccb) == 0) ) {
...
static void process_s_frame (tL2C_CCB *p_ccb, BT_HDR *p_buf, UINT16 ctrl_word)
{
    ...
    if (ctrl_word & L2CAP_FCR_P_BIT) {
        p_fcrb->rej_sent   = 0;             /* After checkpoint, we can send anoher REJ */
        p_fcrb->send_f_rsp = 1;              /* Set a flag in case an I-frame is pending */
    }

    switch (s_frame_type) {
    case L2CAP_FCR_SUP_RR:
        remote_was_busy     = p_fcrb->remote_busy;
        p_fcrb->remote_busy = 0;
...
static void process_i_frame (tL2C_CCB *p_ccb, BT_HDR *p_buf, UINT16 ctrl_word, BOOLEAN delay_ack)
{
    ...
    p_fcrb->rej_sent = p_fcrb->srej_sent = 0;
...
**
** Returns          BOOLEAN - 1 if retransmitted
**
*******************************************************************************/
static BOOLEAN retransmit_i_frames (tL2C_CCB *p_ccb, UINT8 tx_seq)
{
...
BT_HDR *l2c_fcr_get_next_xmit_sdu_seg (tL2C_CCB *p_ccb, UINT16 max_packet_length)
{
    ...
    BOOLEAN     first_seg    = 0,       /* The segment is the first part of data  */
                mid_seg      = 0,       /* The segment is the middle part of data */
                last_seg     = 0;       /* The segment is the last part of data   */
    UINT16      sdu_len = 0;
    BT_HDR      *p_buf, *p_xmit;
    UINT8       *p;
    ...
    */
    ...
    prepare_I_frame (p_ccb, p_xmit, 0);
...
**
** Returns          1 if no errors, Otherwise 0
**
*******************************************************************************/
    ...
    */
    if (p_cfg->fcr_present && !(p_ccb->config_done & RECONFIG_FLAG)) {
        ...
        /* Basic is the only common channel mode between the two devices */
            ...
            p_cfg->fcr_present = 0;
            p_cfg->fcs_present = 0;             /* Illegal to use FCS option in basic mode */
            p_cfg->ext_flow_spec_present = 0;   /* Illegal to use extended flow spec in basic mode */
        }
...
**
** Returns          1 if resent configuration, 0 if channel matches or
**                  cannot match.
**
*******************************************************************************/
BOOLEAN l2c_fcr_renegotiate_chan(tL2C_CCB *p_ccb, tL2CAP_CFG_INFO *p_cfg)
{
    ...
    if (p_cfg->result == L2CAP_CFG_UNACCEPTABLE_PARAMS) {
        ...
        if (p_ccb->our_cfg.fcr.mode != peer_mode) {
            ...
            if (can_renegotiate) {
                p_ccb->our_cfg.fcr_present = 1;

                if (p_ccb->our_cfg.fcr.mode == L2CAP_FCR_BASIC_MODE) {
                    p_ccb->our_cfg.fcs_present = 0;
                    p_ccb->our_cfg.ext_flow_spec_present = 0;
...
UINT8 l2c_fcr_process_peer_cfg_req(tL2C_CCB *p_ccb, tL2CAP_CFG_INFO *p_cfg)
{
    ...
    p_ccb->p_lcb->w4_info_rsp = 0;      /* Handles T61x SonyEricsson Bug in Info Request */
    ...
    if (fcr_ok == L2CAP_PEER_CFG_OK) {
        ...
        if (p_ccb->peer_cfg_already_rejected) {
            ...
            p_ccb->peer_cfg_already_rejected = 1;
        }
    }
