...
BOOLEAN btm_update_dev_to_white_list(BOOLEAN to_add, BD_ADDR bd_addr, tBLE_ADDR_TYPE addr_type, tBTM_ADD_WHITELIST_CBACK *add_wl_cb)
{
    ...
    if (to_add) {
        ...
        if(!background_connection_add((bt_bdaddr_t *)bd_addr)) {
            /* if the bd_addr already exist in whitelist, just callback return 1 */
            if (add_wl_cb){
        ...
        if(!background_connection_remove((bt_bdaddr_t *)bd_addr)){
            /* if the bd_addr don't exist in whitelist, just callback return 1 */
            if (add_wl_cb){
                add_wl_cb(HCI_SUCCESS,to_add);
            }
            return 1;
...
**
** Parameters       start: 1 to start; 0 to stop.
**
** Returns          void
**
*******************************************************************************/
...
#endif
                if (!btsnd_hcic_ble_set_scan_enable(1, 1)) { /* duplicate filtering enabled */
                    return 0;
                }
...
void btm_ble_initiate_select_conn(BD_ADDR bda)
{
    ...
    if (!L2CA_ConnectFixedChnl(L2CAP_ATT_CID, bda, BLE_ADDR_UNKNOWN_TYPE, 0)) {
...
BOOLEAN btm_ble_suspend_bg_conn(void)
{
    ...
    if (btm_cb.ble_ctr_cb.bg_conn_type == BTM_BLE_CONN_AUTO) {
        return btm_ble_start_auto_conn(0);
    } else if (btm_cb.ble_ctr_cb.bg_conn_type == BTM_BLE_CONN_SELECTIVE) {
        return btm_ble_start_select_conn(0, NULL);
    }
...
BOOLEAN btm_ble_resume_bg_conn(void)
{
    ...
    if (p_cb->bg_conn_type != BTM_BLE_CONN_NONE) {
        if (p_cb->bg_conn_type == BTM_BLE_CONN_AUTO) {
            ret = btm_ble_start_auto_conn(1);
        }

        if (p_cb->bg_conn_type == BTM_BLE_CONN_SELECTIVE) {
            ret = btm_ble_start_select_conn(1, btm_cb.ble_ctr_cb.p_select_cback);
        }
    }
