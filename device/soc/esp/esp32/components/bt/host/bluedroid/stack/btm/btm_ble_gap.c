...
static void btm_ble_vendor_capability_vsc_cmpl_cback (tBTM_VSC_CMPL *p_vcs_cplt_params)
{
    ...
    if (status == HCI_SUCCESS) {
        ...
        btm_cb.cmn_ble_vsc_cb.values_read = 1;
    }
...
{
    if (1 == btm_cb.cmn_ble_vsc_cb.values_read) {
        return;
    }
...
BOOLEAN BTM_BleConfigPrivacy(BOOLEAN privacy_mode, tBTM_SET_LOCAL_PRIVACY_CBACK *set_local_privacy_cback)
{
...
#if (defined(GAP_INCLUDED) && GAP_INCLUDED == 1 && GATTS_INCLUDED == 1)
    uint8_t addr_resolution = 0;
#endif  /* defined(GAP_INCLUDED) && GAP_INCLUDED == 1 && GATTS_INCLUDED == 1 */
    if (!privacy_mode) { /* if privacy disabled, always use public address */
        ...
        btsnd_hcic_ble_set_addr_resolution_enable(0);
        ...
        // Disable RPA function
        btsnd_hcic_ble_set_addr_resolution_enable(1);
    }
...
**
** Returns          Return 1 if local privacy is enabled else 0
**
*******************************************************************************/
    ...
    if (btm_cb.ble_ctr_cb.bg_conn_type != bg_conn_type) {
        ...
        case BTM_BLE_CONN_NONE:
            if (btm_cb.ble_ctr_cb.bg_conn_type == BTM_BLE_CONN_AUTO) {
                ...
                btm_ble_start_select_conn(0, NULL);
            }
            started = 1;
            break;
...
**
** Parameters       add_remove: 1 to add; 0 to remove.
...
tBTM_STATUS BTM_BleWriteScanRsp(tBTM_BLE_AD_MASK data_mask, tBTM_BLE_ADV_DATA *p_data)
{
    ...
    if (btsnd_hcic_ble_set_scan_rsp_data((UINT8)(p - rsp_data), rsp_data)) {
        ...
        if (adv_data_status == BTM_SUCCESS && data_mask != 0) {
            btm_cb.ble_ctr_cb.inq_var.scan_rsp = 1;
        } else {
            btm_cb.ble_ctr_cb.inq_var.scan_rsp = 0;
        }
    } else {
        ret = BTM_ILLEGAL_VALUE;
    }
...
BOOLEAN btm_ble_update_inq_result(BD_ADDR bda, tINQ_DB_ENT *p_i, UINT8 addr_type, UINT8 evt_type, UINT8 *p)
{
    ...
    if ((btm_cb.ble_ctr_cb.inq_var.scan_type == BTM_BLE_SCAN_MODE_ACTI &&
        ...
        p_i->scan_rsp = 0;
#if BTM_BLE_ACTIVE_SCAN_REPORT_ADV_SCAN_RSP_INDIVIDUALLY == 0
        to_report = 0;
#endif
    } else {
        p_i->scan_rsp = 1;
...
void btm_ble_process_adv_pkt (UINT8 *p_data)
{
...
#endif//(defined BLE_PRIVACY_SPT && BLE_PRIVACY_SPT == 1)
    ...
    while (num_reports--) {
        ...
        match = btm_identity_addr_to_random_pseudo(bda, &addr_type, 0);
...
#if (defined BLE_PRIVACY_SPT && BLE_PRIVACY_SPT == 1 && (!CONTROLLER_RPA_LIST_ENABLE))
...
{
#if (BLE_ADV_REPORT_FLOW_CONTROL == 1)
    uint32_t num_dis = 0;
...
**
** Returns          operation status. 1 if sucessful, 0 otherwise.
**
*******************************************************************************/
...
void btm_ble_dir_adv_tout(void)
{
    ...
    btm_cb.ble_ctr_cb.inq_var.directed_conn = 0;
}
...
**
** Returns          1 is request is allowed, 0 otherwise.
**
*******************************************************************************/
...
void btm_ble_init (void)
{
    ...
    btm_cb.cmn_ble_vsc_cb.values_read = 0;
    p_cb->cur_states       = 0;
