...
typedef struct {
    ...
    tBTA_AV_HNDL    hndl;
    tBTA_AV_STATUS  status;
    BOOLEAN         initiator; /* 1, if local device initiates the START */
    BOOLEAN         suspending;
} tBTA_AV_START;
...
typedef struct {
    ...
    BOOLEAN avrc_group;         /* 1, to accept AVRC 1.3 group nevigation command */
...
**                  If suspend is 1, this function sends AVDT suspend signal
...
**                  If suspend is 1, this function tries the suspend/reconfigure
**                  procedure first.
**                  If suspend is 0 or when suspend/reconfigure fails,
