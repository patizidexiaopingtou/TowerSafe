...
UINT8 bta_av_sbc_cfg_for_cap(UINT8 *p_peer, tA2D_SBC_CIE *p_cap, tA2D_SBC_CIE *p_pref)
{
    ...
    if ((status = A2D_ParsSbcInfo(&peer_cie, p_peer, 1)) != 0) {
        return status;
    }
...
UINT8 bta_av_sbc_cfg_matches_cap(UINT8 *p_cfg, tA2D_SBC_CIE *p_cap)
{
    ...
    /* parse configuration */
    if ((status = A2D_ParsSbcInfo(&cfg_cie, p_cfg, 1)) != 0) {
...
UINT8 bta_av_sbc_cfg_in_cap(UINT8 *p_cfg, tA2D_SBC_CIE *p_cap)
{
    ...
    /* parse configuration */
    if ((status = A2D_ParsSbcInfo(&cfg_cie, p_cfg, 0)) != 0) {
        return status;
    }
...
void bta_av_sbc_bld_hdr(BT_HDR *p_buf, UINT16 fr_per_pkt)
{
    ...
    A2D_BldSbcMplHdr(p, 0, 0, 0, (UINT8) fr_per_pkt);
}

#endif /* #if defined(BTA_AV_INCLUDED) && (BTA_AV_INCLUDED == 1) */
