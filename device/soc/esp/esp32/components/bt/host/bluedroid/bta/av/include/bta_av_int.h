...
typedef struct {
    ...
    UINT8               handle;
    UINT8               avdt_event;
    BOOLEAN             initiator;  /* 1, if local device initiates the SUSPEND */
} tBTA_AV_STR_MSG;
...
typedef struct {
    ...
    BOOLEAN             cong;           /* 1 if AVDTP congested */
    ...
    BOOLEAN             use_rc;         /* 1 if AVRCP is allowed */
    BOOLEAN             started;        /* 1 if stream started */
    UINT8               co_started;     /* non-zero, if stream started from call-out perspective */
    BOOLEAN             recfg_sup;      /* 1 if the first attempt to reconfigure the stream was successfull, else 0 if command fails */
    BOOLEAN             suspend_sup;    /* 1 if Suspend stream is supported, else 0 if suspend command fails */
    BOOLEAN             deregistring;   /* 1 if deregistering */
    BOOLEAN             sco_suspend;    /* 1 if SUSPEND is issued automatically for SCO */
    ...
    BOOLEAN             no_rtp_hdr;     /* 1 if add no RTP header*/
...
typedef struct {
    ...
    BOOLEAN             disabling;      /* 1 if api disabled called */
    ...
    BOOLEAN             sco_occupied;   /* 1 if SCO is being used or call is in progress */
