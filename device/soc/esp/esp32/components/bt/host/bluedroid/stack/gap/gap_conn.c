...
{
#if ((defined AMP_INCLUDED) && (AMP_INCLUDED == 1))
...
**
** Parameters:      is_server   - If 1, the connection is not created
...
#if (GAP_CONN_POST_EVT_INCLUDED == 1)
...
UINT16 GAP_ConnSetIdleTimeout (UINT16 gap_handle, UINT16 timeout)
{
    ...
    if (L2CA_SetIdleTimeout (p_ccb->connection_id, timeout, 0)) {
        return (BT_PASS);
    } else {
        return (GAP_ERR_BAD_HANDLE);
    }
...
static void gap_connect_cfm (UINT16 l2cap_cid, UINT16 result)
{
    ...
    if ( (p_ccb->con_flags & GAP_CCB_FLAGS_SEC_DONE) == 0) {
        btm_sec_mx_access_request (p_ccb->rem_dev_address, p_ccb->psm, 1,
...
#endif /* (GAP_CONN_POST_EVT_INCLUDED == 1) */
