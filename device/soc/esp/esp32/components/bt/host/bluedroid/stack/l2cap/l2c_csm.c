...
// btla-specific --
        ...
        // 0-POSITIVE error from Coverity test-tool. Please do NOT remove following comment.
        // coverity[uninit_use_in_call] 0-positive: setting the mode to BTM_PM_MD_ACTIVE only uses settings.mode the other data members of tBTM_PM_PWR_MD are ignored
        */
    ...
    if (btm_sec_l2cap_access_req (p_ccb->p_lcb->remote_bd_addr, p_ccb->p_rcb->psm,
                                  p_ccb->p_lcb->handle, 1, &l2c_link_sec_comp, p_ccb) == BTM_CMD_STARTED) {
        p_ccb->chnl_state = CST_ORIG_W4_SEC_COMP;
    }
    break;
...
// btla-specific --
        ...
        if (btm_sec_l2cap_access_req (p_ccb->p_lcb->remote_bd_addr, p_ccb->p_rcb->psm,
                                      p_ccb->p_lcb->handle, 0, &l2c_link_sec_comp, p_ccb) == BTM_CMD_STARTED) {
...
static void l2c_csm_term_w4_sec_comp (tL2C_CCB *p_ccb, UINT16 event, void *p_data)
{
    ...
    case L2CEVT_SEC_RE_SEND_CMD:                    /* BTM has enough info to proceed */
        btm_sec_l2cap_access_req (p_ccb->p_lcb->remote_bd_addr, p_ccb->p_rcb->psm,
                                  p_ccb->p_lcb->handle, 0, &l2c_link_sec_comp, p_ccb);
        break;
    }
}
...
static void l2c_csm_config (tL2C_CCB *p_ccb, UINT16 event, void *p_data)
{
    ...
    case L2CEVT_L2CAP_CONFIG_RSP_NEG:              /* Peer config error rsp */
        ...
        if (l2c_fcr_renegotiate_chan(p_ccb, p_cfg) == 0) {
    ...
    case L2CEVT_L2CAP_DISCONNECT_REQ:                  /* Peer disconnected request */
        ...
        (*p_ccb->p_rcb->api.pL2CA_DisconnectInd_Cb)(p_ccb->local_cid, 1);
        break;
...
static void l2c_csm_open (tL2C_CCB *p_ccb, UINT16 event, void *p_data)
{
    ...
    case L2CEVT_LP_DISCONNECT_IND:                  /* Link was disconnected */
        ...
        l2cu_release_ccb (p_ccb);
        if (p_ccb->p_rcb) {
            (*p_ccb->p_rcb->api.pL2CA_DisconnectInd_Cb)(local_cid, 0);
        }
        break;
