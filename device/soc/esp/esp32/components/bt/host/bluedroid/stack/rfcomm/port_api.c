...
**                  is_server    - 1 if requesting application is a server
...
**
** Description      This function returns 1 if there is any RFCOMM connection
**                  opening in process.
**
** Parameters:      1 if any connection opening is found
...
int PORT_FlowControl (UINT16 handle, BOOLEAN enable)
{
    ...
    if (p_port->rfc.p_mcb->flow == PORT_FC_CREDIT) {
        if (!p_port->rx.user_fc) {
            port_flow_control_peer(p_port, 1, 0);
        }
    } else {
...
int PORT_FlowControl_MaxCredit (UINT16 handle, BOOLEAN enable)
{
    ...
    if (p_port->rfc.p_mcb->flow == PORT_FC_CREDIT) {
        if (!p_port->rx.user_fc) {
            port_flow_control_peer(p_port, 1, p_port->credit_rx);
        }
    } else {
        old_fc = p_port->local_ctrl.fc;
...
int PORT_FlowControl_GiveCredit (UINT16 handle, BOOLEAN enable, UINT16 credits_given)
{
    ...
    if (p_port->rfc.p_mcb->flow == PORT_FC_CREDIT) {
        if (!p_port->rx.user_fc) {
            port_flow_control_peer(p_port, 1, credits_given);
        }
    } else {
...
int PORT_Purge (UINT16 handle, UINT8 purge_flags)
{
    ...
    if (purge_flags & PORT_PURGE_RXCLEAR) {
        ...
        if (count) {
            port_flow_control_peer (p_port, 1, count);
        }
    }
...
int PORT_Test (UINT16 handle, UINT8 *p_data, UINT16 len)
{
    ...
    if ((p_buf = (BT_HDR *)osi_malloc(RFCOMM_CMD_BUF_SIZE)) != NULL) {
        ...
        rfc_send_test (p_port->rfc.p_mcb, 1, p_buf);
        return (PORT_SUCCESS);
    } else {
        return (PORT_NO_MEM);
    }
...
#endif ///(defined RFCOMM_INCLUDED && RFCOMM_INCLUDED == 1)
