...
#endif  ///CLASSIC_BT_GATT_INCLUDED == 1
...
#endif  ///CLASSIC_BT_GATT_INCLUDED == 1
    BTM_SetSecurityLevel(1, "", BTM_SEC_SERVICE_ATT, BTM_SEC_NONE, BT_PSM_ATT, 0, 0);
    BTM_SetSecurityLevel(0, "", BTM_SEC_SERVICE_ATT, BTM_SEC_NONE, BT_PSM_ATT, 0, 0);
...
** Returns          1 if connection is started, otherwise return 0.
**
*******************************************************************************/
...
**
** Returns          1: if connection found and to be disconnected; otherwise
**                  return 0.
**
*******************************************************************************/
                                    ...
                                    tBLE_ADDR_TYPE bd_addr_type, tBT_TRANSPORT transport, BOOLEAN is_aux)
{
    ...
    if (ret) {
        gatt_update_app_use_link_flag(p_reg->gatt_if, p_tcb, 1, 0);
    }
...
**                      connected (conn = 1)/disconnected (conn = 0).
**
*******************************************************************************/
static void gatt_le_connect_cback (UINT16 chan, BD_ADDR bd_addr, BOOLEAN connected,
                                   UINT16 reason, tBT_TRANSPORT transport)
    ...
    for (i = 0,  p_reg = gatt_cb.cl_rcb ; i < GATT_MAX_APPS; i++, p_reg++) {
        if (p_reg->in_use) {
            if (p_bg_dev && gatt_is_bg_dev_for_app(p_bg_dev, p_reg->gatt_if)) {
                gatt_update_app_use_link_flag(p_reg->gatt_if, p_tcb, 1, 1);
            }
...
#if (GATTS_INCLUDED == 1) || (GATTC_INCLUDED == 1)
    UINT16  msg_len;
#endif ///(GATTS_INCLUDED == 1) || (GATTC_INCLUDED == 1)
    ...
    if (p_buf->len > 0) {
        ...
        if (pseudo_op_code < GATT_OP_CODE_MAX) {
            ...
            if (op_code & GATT_COMMAND_FLAG) {
                ...
                gatt_send_error_rsp (p_tcb, GATT_REQ_NOT_SUPPORTED, op_code, 0, 0);
            }
        }
    } else {
...
void gatt_add_a_bonded_dev_for_srv_chg (BD_ADDR bda)
{
    ...
    srv_chg_clt.srv_changed = 0;
    if (gatt_add_srv_chg_clt(&srv_chg_clt) != NULL) {
        memcpy(req.srv_chg.bda, bda, BD_ADDR_LEN);
        req.srv_chg.srv_changed = 0;
    ...
    if (gatt_cb.cb_info.p_srv_chg_callback) {
        ...
        if (status && rsp.num_clients) {
            ...
            while ((i <= num_clients) && status) {
                ...
                if ((status = (*gatt_cb.cb_info.p_srv_chg_callback)(GATTS_SRV_CHG_CMD_READ_CLENT, &req, &rsp)) == 1) {
