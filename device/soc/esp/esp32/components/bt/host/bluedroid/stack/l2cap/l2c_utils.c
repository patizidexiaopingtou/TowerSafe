...
**
** Returns          BOOLEAN - 1 if cmd_code is a command and it is rejected,
**                            0 if response code. (command not rejected)
**
*******************************************************************************/
...
#endif
    {
#if (L2CAP_NON_FLUSHABLE_PB_INCLUDED == 1)
...
#if (L2CAP_CONFORMANCE_TESTING == 1)
...
#if (L2CAP_ROUND_ROBIN_CHANNEL_SERVICE == 1)
...
tL2C_CCB *l2cu_allocate_ccb (tL2C_LCB *p_lcb, UINT16 cid)
{
    ...
    p_ccb->peer_cfg_already_rejected = 0;
...
**
** Returns          BOOLEAN  - 1 if idle timer started or disconnect initiated
**                             0 if there's one or more pending CCB's exist
**
*******************************************************************************/
...
{
    BOOLEAN  mtu_ok      = 1;
    BOOLEAN  qos_type_ok = 1;
    BOOLEAN  flush_to_ok = 1;
    BOOLEAN  fcr_ok      = 1;
#if (CLASSIC_BT_INCLUDED == 1)
    UINT8    fcr_status;
#endif  ///CLASSIC_BT_INCLUDED == 1
    ...
    if (p_cfg->flush_to_present) {
        if (!p_cfg->flush_to) {
            ...
            p_ccb->peer_cfg.flush_to_present = 1;
...
**
** Returns          1 if successful, 0 if gki get buffer fails.
**
*******************************************************************************/
...
**
** Returns          1 if successful, 0 if osi get buffer fails.
**
*******************************************************************************/
...
**
** Returns          1 if any of above conditions met, 0 otherwise
**
*******************************************************************************/
...
**
** Returns          1 if a valid channel, else 0
**
*******************************************************************************/
        ...
        if (l2cb.fixed_reg[xx].pL2CA_FixedConn_Cb != NULL) {
            if (p_lcb->peer_chnl_mask[(xx + L2CAP_FIRST_FIXED_CHNL) / 8]
                ...
                (*l2cb.fixed_reg[xx].pL2CA_FixedConn_Cb)(xx + L2CAP_FIRST_FIXED_CHNL,
                        p_lcb->remote_bd_addr, 1, 0, p_lcb->transport);
#else
                (*l2cb.fixed_reg[xx].pL2CA_FixedConn_Cb)(xx + L2CAP_FIRST_FIXED_CHNL,
                        p_lcb->remote_bd_addr, 1, 0, BT_TRANSPORT_BR_EDR);
#endif
            } else {
#if BLE_INCLUDED == 1
                (*l2cb.fixed_reg[xx].pL2CA_FixedConn_Cb)(xx + L2CAP_FIRST_FIXED_CHNL,
                        p_lcb->remote_bd_addr, 0, p_lcb->disc_reason, p_lcb->transport);
#else
                (*l2cb.fixed_reg[xx].pL2CA_FixedConn_Cb)(xx + L2CAP_FIRST_FIXED_CHNL,
                        p_lcb->remote_bd_addr, 0, p_lcb->disc_reason, BT_TRANSPORT_BR_EDR);
#endif
...
{
    tL2C_CCB    *p_ccb = NULL;
#if (L2CAP_UCD_INCLUDED == 0)
    if (local_cid < L2CAP_BASE_APPL_CID) {
        return NULL;
    }
#endif //(L2CAP_UCD_INCLUDED == 0)
    list_node_t *p_node = NULL;
...
#if (L2CAP_ROUND_ROBIN_CHANNEL_SERVICE == 1 && CLASSIC_BT_INCLUDED == 1)
...
#else /* (L2CAP_ROUND_ROBIN_CHANNEL_SERVICE == 1) */
...
#endif /* (L2CAP_ROUND_ROBIN_CHANNEL_SERVICE == 1) */
        ...
        if ( p_ccb->cong_sent ) {
            ...
            if (q_count <= (p_ccb->buff_quota / 2)) {
                ...
                if (p_ccb->p_rcb && p_ccb->p_rcb->api.pL2CA_CongestionStatus_Cb) {
                    L2CAP_TRACE_DEBUG ("L2CAP - Calling CongestionStatus_Cb (0), CID: 0x%04x  xmit_hold_q.count: %u  buff_quota: %u",
                                       p_ccb->local_cid, q_count, p_ccb->buff_quota);

                    /* Prevent recursive calling */
                    l2cb.is_cong_cback_context = 1;
                    (*p_ccb->p_rcb->api.pL2CA_CongestionStatus_Cb)(p_ccb->local_cid, 0);
                    l2cb.is_cong_cback_context = 0;
                }
#if (L2CAP_UCD_INCLUDED == 1)
                else if ( p_ccb->p_rcb && p_ccb->local_cid == L2CAP_CONNECTIONLESS_CID ) {
                    if ( p_ccb->p_rcb->ucd.cb_info.pL2CA_UCD_Congestion_Status_Cb ) {
                        L2CAP_TRACE_DEBUG ("L2CAP - Calling UCD CongestionStatus_Cb (0), SecPendingQ:%u,XmitQ:%u,Quota:%u",
                                           fixed_queue_length(p_ccb->p_lcb->ucd_out_sec_pending_q),
                                           fixed_queue_length(p_ccb->xmit_hold_q),
                                           p_ccb->buff_quota);
                        p_ccb->p_rcb->ucd.cb_info.pL2CA_UCD_Congestion_Status_Cb( p_ccb->p_lcb->remote_bd_addr, 0 );
                    }
                }
#endif
#if (L2CAP_NUM_FIXED_CHNLS > 0)
                    ...
                    for (xx = 0; xx < L2CAP_NUM_FIXED_CHNLS; xx ++) {
                        if (p_ccb->p_lcb->p_fixed_ccbs[xx] == p_ccb) {
                            if (l2cb.fixed_reg[xx].pL2CA_FixedCong_Cb != NULL) {
                                (* l2cb.fixed_reg[xx].pL2CA_FixedCong_Cb)(p_ccb->p_lcb->remote_bd_addr, 0);
                            }
                            break;
                        }
                    }
            ...
            if (q_count > p_ccb->buff_quota || (p_lcb && (p_lcb->link_xmit_data_q) && (list_length(p_lcb->link_xmit_data_q) + q_count) > p_ccb->buff_quota)) {
                p_ccb->cong_sent = 1;
                if (p_ccb->p_rcb && p_ccb->p_rcb->api.pL2CA_CongestionStatus_Cb) {
                    L2CAP_TRACE_DEBUG ("L2CAP - Calling CongestionStatus_Cb (1),CID:0x%04x,XmitQ:%u,Quota:%u",
                                       p_ccb->local_cid, q_count, p_ccb->buff_quota);

                    (*p_ccb->p_rcb->api.pL2CA_CongestionStatus_Cb)(p_ccb->local_cid, 1);
                }
#if (L2CAP_UCD_INCLUDED == 1)
                else if ( p_ccb->p_rcb && p_ccb->local_cid == L2CAP_CONNECTIONLESS_CID ) {
                    if ( p_ccb->p_rcb->ucd.cb_info.pL2CA_UCD_Congestion_Status_Cb ) {
                        L2CAP_TRACE_DEBUG ("L2CAP - Calling UCD CongestionStatus_Cb (1), SecPendingQ:%u,XmitQ:%u,Quota:%u",
                                          fixed_queue_length(p_ccb->p_lcb->ucd_out_sec_pending_q),
                                          fixed_queue_length(p_ccb->xmit_hold_q),
                                          p_ccb->buff_quota);
                        p_ccb->p_rcb->ucd.cb_info.pL2CA_UCD_Congestion_Status_Cb( p_ccb->p_lcb->remote_bd_addr, 1 );
                    }
                }
#endif
#if (L2CAP_NUM_FIXED_CHNLS > 0)
                    ...
                    for (xx = 0; xx < L2CAP_NUM_FIXED_CHNLS; xx ++) {
                        if (p_ccb->p_lcb->p_fixed_ccbs[xx] == p_ccb) {
                            if (l2cb.fixed_reg[xx].pL2CA_FixedCong_Cb != NULL) {
                                (* l2cb.fixed_reg[xx].pL2CA_FixedCong_Cb)(p_ccb->p_lcb->remote_bd_addr, 1);
                            }
                            break;
                        }
                    }
