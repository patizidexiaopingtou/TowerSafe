 ...
 **
 ** Returns          1 if the current streaming has CP, 0 otherwise
 **
 *******************************************************************************/
 ...
 **                  BTA_AV_CP_SCMS_COPY_FREE
 **
 ** Returns          1 if setting the SCMS flag is supported else 0
 **
 *******************************************************************************/
...
#if defined(BTA_AV_CO_CP_SCMS_T) && (BTA_AV_CO_CP_SCMS_T == 1)
#else
...
void bta_av_build_src_cfg (UINT8 *p_pref_cfg, UINT8 *p_src_cap)
{
    ...
    /* parse configuration */
    if ((status = A2D_ParsSbcInfo(&src_cap, p_src_cap, 1)) != 0) {
                ...
                *p_num_protect = 0;
                ...
                if (p_peer->acp) {
                    if (p_peer->recfg_needed) {
                        ...
                        BTA_AvReconfig(hndl, 1, p_sink->sep_info_idx, p_peer->codec_cfg, *p_num_protect, (UINT8 *)bta_av_co_cp_scmst);
                    }
                } else {
                               ...
                               UINT8 t_local_sep, UINT8 avdt_handle)
{
    ...
    BOOLEAN recfg_needed = 0;
    BOOLEAN codec_cfg_supported = 0;
    UNUSED(seid);
    UNUSED(addr);
    ...
    if (p_peer == NULL) {
        ...
        bta_av_ci_setconfig(hndl, A2D_BUSY, AVDT_ASC_CODEC, 0, NULL, 0, avdt_handle);
        return;
    }
    ...
    if (num_protect != 0) {
        /* If CP is supported */
        if ((num_protect != 1) ||
                (bta_av_co_cp_is_scmst(p_protect_info) == 0)) {
    ...
    if (status != A2D_SUCCESS) {
        ...
        /* Call call-in rejecting the configuration */
        bta_av_ci_setconfig(hndl, status, category, 0, NULL, 0, avdt_handle);
    } else {
 ...
 **
 ** Returns          1 if the codec was built successfully, 0 otherwise
 **
 *******************************************************************************/
static BOOLEAN bta_av_co_audio_codec_build_config(const UINT8 *p_codec_caps, UINT8 *p_codec_cfg)
{
 ...
 **
 ** Returns          1 if it codec config is supported, 0 otherwise
 **
 *******************************************************************************/
 ...
 **
 ** Returns          1 if the connection supports this codec, 0 otherwise
 **
 *******************************************************************************/
 ...
 **
 ** Returns          1 if this CP is SCMS-T, 0 otherwise
 **
 *******************************************************************************/
static BOOLEAN bta_av_co_cp_is_scmst(const UINT8 *p_protectinfo)
{
 ...
 **
 ** Returns          1 if the sink supports this CP, 0 otherwise
 **
 *******************************************************************************/
static BOOLEAN bta_av_co_audio_sink_has_scmst(const tBTA_AV_CO_SINK *p_sink)
{
 ...
 **
 ** Returns          1 if the media source supports this config, 0 otherwise
 **
 *******************************************************************************/
static BOOLEAN bta_av_co_audio_sink_supports_config(UINT8 codec_type, const UINT8 *p_codec_cfg)
{
 ...
 **
 ** Returns          1 if all opened devices support this codec, 0 otherwise
 **
 *******************************************************************************/
            ...
            if (bta_av_co_audio_peer_supports_codec(p_peer, &snk_index)) {
                ...
                if (bta_av_co_audio_codec_build_config(p_sink->codec_caps, codec_cfg)) {
                    ...
                    if ((p_sink != p_peer->p_snk) ||
                        ...
                        BTA_AvReconfig(BTA_AV_CO_AUDIO_INDX_TO_HNDL(index), 1, p_sink->sep_info_idx,
 ...
 **
 ** Returns          1 if codec is SBC, 0 otherwise
 **
 *******************************************************************************/
BOOLEAN bta_av_co_audio_get_sbc_config(tA2D_SBC_CIE *p_sbc_config, UINT16 *p_minmtu)
{
    ...
    *p_minmtu = 0xFFFF;
    ...
    if (bta_av_co_cb.codec_cfg.id == BTC_AV_CODEC_SBC) {
        if (A2D_ParsSbcInfo(p_sbc_config, bta_av_co_cb.codec_cfg.info, 0) == A2D_SUCCESS) {
            for (index = 0; index < BTA_AV_CO_NUM_ELEMENTS(bta_av_co_cb.peers); index++) {
                p_peer = &bta_av_co_cb.peers[index];
                if (p_peer->opened) {
 ...
 **
 ** Returns          1 if the peer supports CP
 **
 *******************************************************************************/
 ...
 **                  remote preference.
 **
 ** Returns          1 if config set, 0 otherwize
 **
 *******************************************************************************/
