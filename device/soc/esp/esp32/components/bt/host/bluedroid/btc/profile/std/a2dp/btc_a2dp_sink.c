...
{
    a2dp_sink_local_param.btc_aa_snk_cb.rx_flush = 1;
...
static void btc_a2dp_sink_data_ready(UNUSED_ATTR void *context)
{
    ...
    if (fixed_queue_is_empty(a2dp_sink_local_param.btc_aa_snk_cb.RxSbcQ)) {
        ...
        if (a2dp_sink_local_param.btc_aa_snk_cb.rx_flush == 1) {
...
static void btc_a2dp_sink_handle_decoder_reset(tBTC_MEDIA_SINK_CFG_UPDATE *p_msg)
{
    ...
    a2d_status = A2D_ParsSbcInfo(&sbc_cie, p_buf->codec_info, 0);
    ...
    a2dp_sink_local_param.btc_aa_snk_cb.rx_flush = 0;
    ...
    status = OI_CODEC_SBC_DecoderReset(&a2dp_sink_local_param.context, a2dp_sink_local_param.contextData,
                                        sizeof(a2dp_sink_local_param.contextData), 2, 2, 0, 0);
    if (!OI_SUCCESS(status)) {
        APPL_TRACE_ERROR("OI_CODEC_SBC_DecoderReset failed with error code %d\n", status);
    }

    btc_a2dp_control_set_datachnl_stat(1);
...
{
    if (fixed_queue_is_empty(a2dp_sink_local_param.btc_aa_snk_cb.RxSbcQ) == 1) { /*  Que is already empty */
        return 1;
    }
...
UINT8 btc_a2dp_sink_enque_buf(BT_HDR *p_pkt)
{
    ...
    if (a2dp_sink_local_param.btc_aa_snk_cb.rx_flush == 1) { /* Flush enabled, do not enque*/
