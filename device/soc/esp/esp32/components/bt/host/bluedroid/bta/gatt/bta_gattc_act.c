...
#if (defined BTA_HH_LE_INCLUDED && BTA_HH_LE_INCLUDED == 1)
...
#if GATTC_INCLUDED == 1 && BLE_INCLUDED == 1
        ...
        for (i = 0; i < BTA_GATTC_KNOWN_SR_MAX; i ++) {
            if (p_cb->bg_track[i].in_use) {
                if (p_cb->bg_track[i].cif_mask & (1 << (p_clreg->client_if - 1))) {
                    bta_gattc_mark_bg_conn(p_clreg->client_if, p_cb->bg_track[i].remote_bda, 0, 0);
                    GATT_CancelConnect(p_clreg->client_if, p_cb->bg_track[i].remote_bda, 0);
                }
                if (p_cb->bg_track[i].cif_adv_mask & (1 << (p_clreg->client_if - 1))) {
                    bta_gattc_mark_bg_conn(p_clreg->client_if, p_cb->bg_track[i].remote_bda, 0, 1);
                }
            }
        }
...
void bta_gattc_open(tBTA_GATTC_CLCB *p_clcb, tBTA_GATTC_DATA *p_data)
{
    ...
    if (!GATT_Connect(p_clcb->p_rcb->client_if, p_data->api_conn.remote_bda, p_data->api_conn.remote_addr_type,
                      1, p_data->api_conn.transport, p_data->api_conn.is_aux)) {
...
void bta_gattc_init_bk_conn(tBTA_GATTC_API_OPEN *p_data, tBTA_GATTC_RCB *p_clreg)
{
    ...
    if (bta_gattc_mark_bg_conn(p_data->client_if, p_data->remote_bda, 1, 0)) {
        /* always call open to hold a connection */
        if (!GATT_Connect(p_data->client_if, p_data->remote_bda,
                          p_data->remote_addr_type, 0,
...
void bta_gattc_cancel_bk_conn(tBTA_GATTC_API_CANCEL_OPEN *p_data)
{
    ...
    if (bta_gattc_mark_bg_conn(p_data->client_if, p_data->remote_bda, 0, 0)) {
        if (GATT_CancelConnect(p_data->client_if, p_data->remote_bda, 0)) {
...
void bta_gattc_cancel_open(tBTA_GATTC_CLCB *p_clcb, tBTA_GATTC_DATA *p_data)
{
    ...
    if (GATT_CancelConnect(p_clcb->p_rcb->client_if, p_data->api_cancel_conn.remote_bda, 1)) {
...
void bta_gattc_conn(tBTA_GATTC_CLCB *p_clcb, tBTA_GATTC_DATA *p_data)
{
    ...
    if (p_clcb->p_rcb) {
        ...
        if (p_data && p_data->int_conn.already_connect) {
            ...
            p_data->int_conn.already_connect = 0;
...
void bta_gattc_start_discover(tBTA_GATTC_CLCB *p_clcb, tBTA_GATTC_DATA *p_data)
{
    ...
    if (((p_clcb->p_q_cmd == NULL || p_clcb->auto_update == BTA_GATTC_REQ_WAITING) &&
        ...
        if (p_clcb->p_srcb != NULL) {
            ...
            #if (BT_MULTI_CONNECTION_ENBALE == 0)
            if (p_clcb->transport == BTA_TRANSPORT_LE) {
                L2CA_EnableUpdateBleConnParams(p_clcb->p_srcb->server_bda, 0);
            }
            #endif
...
void bta_gattc_write_cmpl(tBTA_GATTC_CLCB *p_clcb, tBTA_GATTC_OP_CMPL *p_data)
{
    ...
    if (p_conn && p_conn->svc_change_descr_handle == cb_data.write.handle) {
        if(cb_data.write.status != BTA_GATT_OK) {
            p_conn->write_remote_svc_change_ccc_done = 0;
...
void bta_gattc_process_api_cache_assoc(tBTA_GATTC_CB *p_cb, tBTA_GATTC_DATA *p_msg)
{
    ...
    if (p_msg->api_assoc.is_assoc) {
        if ((state = bta_gattc_co_cache_append_assoc_addr(p_msg->api_assoc.src_addr, p_msg->api_assoc.assoc_addr)) == 1) {
        ...
        if (( state = bta_gattc_co_cache_remove_assoc_addr(p_msg->api_assoc.src_addr, p_msg->api_assoc.assoc_addr)) == 1) {
            gattc_cb.set_assoc.status = BTA_GATT_OK;
        } else {
            gattc_cb.set_assoc.status = BTA_GATT_ERROR;
            if (p_clrcb != NULL) {
                                       ...
                                       tGATT_VALUE *att_value)
{
    ...
    if (p_char && bta_gattc_uuid_compare(&p_char->service->uuid, &gattp_uuid, 1) &&
        bta_gattc_uuid_compare(&p_char->uuid, &srvc_chg_uuid, 1)) {
                                     ...
                                     tBTA_GATTC_NOTIFY *p_notify)
{
    ...
    p_notify->is_notify = (op == GATTC_OPTYPE_INDICATION) ? 0 : 1;
...
void bta_gattc_init_clcb_conn(UINT8 cif, BD_ADDR remote_bda)
{
    ...
    if (GATT_GetConnIdIfConnected(cif, remote_bda, &conn_id, BTA_GATT_TRANSPORT_LE) == 0) {
    ...
    if ((p_clcb = bta_gattc_clcb_alloc(cif, remote_bda, BTA_GATT_TRANSPORT_LE)) != NULL) {
        ...
        gattc_data.api_conn.is_direct = 1;
...
tBTA_GATTC_FIND_SERVICE_CB bta_gattc_register_service_change_notify(UINT16 conn_id, BD_ADDR remote_bda)
{
    ...
    BOOLEAN             gatt_cache_found = 0;
    BOOLEAN             gatt_service_found = 0;
    BOOLEAN             gatt_service_change_found = 0;
    BOOLEAN             gatt_ccc_found = 0;
    ...
    if (gatt_cache_found == 1) {
        for (list_node_t *sn = list_begin(p_cache);
             sn != list_end(p_cache); sn = list_next(sn)) {
            p_service = list_node(sn);
            if (bta_gattc_uuid_compare(&gatt_service_uuid, &p_service->uuid, 1)) {
                gatt_service_found = 1;
                break;
            }
        }
    }
    ...
    if (gatt_service_found == 1) {
        if (p_service->characteristics) {
            for (list_node_t *cn = list_begin(p_service->characteristics);
                ...
                p_char = list_node(cn);
                if (bta_gattc_uuid_compare(&gatt_service_change_uuid, &p_char->uuid, 1)) {
                    gatt_service_change_found = 1;
                    break;
                }
            }
        }
    }
    else if (gatt_cache_found == 1) {
    ...
    if (gatt_ccc_found == 1){
        if (p_conn) {
            ...
            p_conn->write_remote_svc_change_ccc_done = 1;
        }
    ...
    else if (gatt_service_change_found == 1) {
...
#endif
#endif  ///GATTC_INCLUDED == 1 && BLE_INCLUDED == 1
