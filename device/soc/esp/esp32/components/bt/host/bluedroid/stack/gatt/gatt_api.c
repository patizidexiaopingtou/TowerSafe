...
#if defined(BTA_GATT_INCLUDED) && (BTA_GATT_INCLUDED == 1)
...
**
** Returns          1 if handle range is added successfully; otherwise 0.
**
*******************************************************************************/
    ...
    if ((p_list = gatt_find_hdl_buffer_by_app_id(p_app_uuid128, p_svc_uuid, svc_inst)) != NULL) {
        ...
        if (save_hdl) {
            if (gatt_cb.cb_info.p_nv_save_callback) {
                (*gatt_cb.cb_info.p_nv_save_callback)(1, &p_list->asgn_range);
            }
...
**                  svc_inst      : instance of the service inside the application
**
** Returns          1 if operation succeed, 0 if handle block was not found.
**
*******************************************************************************/
BOOLEAN GATTS_DeleteService (tGATT_IF gatt_if, tBT_UUID *p_svc_uuid, UINT16 svc_inst)
{
    ...
    if ( (p_list->asgn_range.s_handle >= gatt_cb.hdl_cfg.app_start_hdl)
        ...
        (*gatt_cb.cb_info.p_nv_save_callback)(0, &p_list->asgn_range);
    }
...
tGATT_STATUS GATTC_Read (UINT16 conn_id, tGATT_READ_TYPE type, tGATT_READ_PARAM *p_read)
{
    ...
    if ( (p_clcb = gatt_clcb_alloc(conn_id)) != NULL  ) {
        ...
        if (gatt_security_check_start(p_clcb) == 0) {
            status = GATT_NO_RESOURCES;
            gatt_clcb_dealloc(p_clcb);
        }
    } else {
...
void GATT_SetIdleTimeout (BD_ADDR bd_addr, UINT16 idle_tout, tBT_TRANSPORT transport)
{
    ...
    if ((p_tcb = gatt_find_tcb_by_addr (bd_addr, transport)) != NULL) {
        if (p_tcb->att_lcid == L2CAP_ATT_CID) {
            ...
            status = L2CA_SetIdleTimeout (p_tcb->att_lcid, idle_tout, 0);
        }
    }
...
#if (defined(BLE_PERIPHERAL_MODE_SUPPORT) && (BLE_PERIPHERAL_MODE_SUPPORT == 1))
    GATT_Listen(gatt_if, 0, NULL);
#endif
    ...
    if ((p_reg = gatt_get_regcb(gatt_if)) != NULL) {
        ...
        while (gatt_find_the_connected_bda(start_idx, bda, &found_idx, &transport)) {
            ...
            if (p_reg->app_cb.p_conn_cb && p_tcb) {
                ...
                (*p_reg->app_cb.p_conn_cb)(gatt_if, bda, conn_id, 1, 0, transport);
            }
            start_idx = ++found_idx;
        }
    }
                              ...
                              BOOLEAN is_direct, tBT_TRANSPORT transport, BOOLEAN is_aux)
{
    ...
    if (is_direct) {
        ...
        if (transport == BT_TRANSPORT_LE) {
            status = gatt_update_auto_connect_dev(gatt_if, 1, bd_addr, 1);
        } else {
...
**
** Returns          1 the logical link information is found for conn_id
**
*******************************************************************************/
...
**
** Returns          1 the logical link is connected
**
*******************************************************************************/
...
**
** Returns          1 if advertisement is started; 0 if adv start failure.
**
*******************************************************************************/
BOOLEAN GATT_Listen (tGATT_IF gatt_if, BOOLEAN start, BD_ADDR_PTR bd_addr)
{
    ...
    if (bd_addr != NULL) {
        gatt_update_auto_connect_dev(gatt_if, start, bd_addr, 0);
    } else {
