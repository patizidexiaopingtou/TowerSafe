...
#if BT_BLE_DYNAMIC_ENV_MEMORY == 0
...
#if (C2H_FLOW_CONTROL_INCLUDED == 1)
...
#if (BLE_50_FEATURE_SUPPORT == 1 && BLE_42_FEATURE_SUPPORT == 0)
        if (HCI_LE_ENHANCED_PRIVACY_SUPPORTED(controller_param.features_ble.as_array)) {
...
#endif // (BLE_50_FEATURE_SUPPORT == 1 && BLE_42_FEATURE_SUPPORT == 0)
...
const controller_t *controller_get_interface(void)
{
    ...
    if (!loaded) {
        loaded = 1;
#if (BT_BLE_DYNAMIC_ENV_MEMORY == 1)
