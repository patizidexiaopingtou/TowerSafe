...
#if (defined(SBC_DEC_INCLUDED) && SBC_DEC_INCLUDED == 1)
                               ...
                               OI_UINT32 *frameBytes)
{
    ...
    if (context->limitFrameFormat && context->enhancedEnabled) {
        ...
        search1 = search2;
    } else if (context->enhancedEnabled == 0) {
    ...
    if (*frameBytes) {
         ...
         * reflects the number of bytes available to read, including the
         * syncword. */
        context->common.frameInfo.enhanced = 0;
        return OI_OK;
    } else {
        /* No syncword was found anywhere in the provided input data.
         * *frameData points past the end of the original input, and
                                     ...
                                     OI_UINT32 *pcmBytes)
{
    ...
    /*
     * Decode the SBC data. Pass 1 to DecodeBody to allow partial decoding of
     * tones.
     */
    status = DecodeBody(context, *frameData, pcmData, pcmBytes, 1);
                                   ...
                                   OI_UINT32 *pcmBytes)
{
     ...
     * stream, so pass 0 to decode body to have it enforce the old rule that
     * you have to decode a whole packet at a time.
     */
    status = DecodeBody(context, *frameData + SBC_HEADER_LEN, pcmData, pcmBytes, 0);
    if (OI_SUCCESS(status)) {
        *frameData += framelen;
        *frameBytes -= framelen;
    }
...
#endif /* #if (defined(SBC_DEC_INCLUDED) && SBC_DEC_INCLUDED == 1) */
