...
static BOOLEAN temp_enhanced = 0;
...
BOOLEAN BTM_SecAddBleKey (BD_ADDR bd_addr, tBTM_LE_KEY_VALUE *p_le_key, tBTM_LE_KEY_TYPE key_type)
{
    ...
    btm_sec_save_le_key (bd_addr, key_type, p_le_key, 0);
...
**                  for an LE link
**
** Returns          1 if connection is LE link, otherwise 0.
**
*******************************************************************************/
...
**
** Returns          BOOLEAN , 1 if connection to remote device exists, else 0
**
*******************************************************************************/
...
**
**
** Parameters:      add          - set static passkey when add is 1
**                                 clear static passkey when add is 0
...
**
** Return           1 if an active link is identified; 0 otherwise
**
*******************************************************************************/
...
void BTM_BleReceiverTest(UINT8 rx_freq, tBTM_CMPL_CB *p_cmd_cmpl_cback)
{
    ...
    if (btsnd_hcic_ble_receiver_test(rx_freq) == 0) {
...
{
    btm_cb.devcb.p_le_test_cmd_cmpl_cb = p_cmd_cmpl_cback;
    if (btsnd_hcic_ble_transmitter_test(tx_freq, test_data_len, packet_payload) == 0) {
...
void BTM_BleEnhancedReceiverTest(UINT8 rx_freq, UINT8 phy, UINT8 modulation_index, tBTM_CMPL_CB *p_cmd_cmpl_cback)
{
    ...
    if (btsnd_hcic_ble_enhand_rx_test(rx_freq, phy, modulation_index) == 0) {
...
{
    btm_cb.devcb.p_le_test_cmd_cmpl_cb = p_cmd_cmpl_cback;
    if (btsnd_hcic_ble_enhand_tx_test(tx_freq, test_data_len, packet_payload, phy) == 0) {
...
**
** Returns          1 to use LE, 0 use BR/EDR.
**
*******************************************************************************/
...
tBTM_SEC_ACTION btm_ble_determine_security_act(BOOLEAN is_originator, BD_ADDR bdaddr, UINT16 security_required)
{
    ...
    BOOLEAN is_link_encrypted = 0;
    BOOLEAN is_key_mitm = 0;
...
**                  is_originator: 1 if outgoing connection.
**                  p_callback : Pointer to the callback function.
**                  p_ref_data : Pointer to be returned along with the callback.
**
** Returns          1 if link already meets the required security; otherwise 0.
**
*******************************************************************************/
...
**
** Returns          1 - if a valid DIV is availavle
*******************************************************************************/
...
**
** Returns          1: check is OK and the *p_sec_req_act contain the action
**
*******************************************************************************/
...
tBTM_STATUS btm_ble_set_encryption (BD_ADDR bd_addr, void *p_ref_data, UINT8 link_role)
{
    ...
    case BTM_BLE_SEC_ENCRYPT:
        if (link_role == BTM_ROLE_MASTER && (p_rec->ble.key_type & BTM_LE_KEY_PENC)) {
            ...
            cmd = btm_ble_start_encrypt(bd_addr, 0, NULL);
            break;
        }
...
#if (SMP_SLAVE_CON_PARAMS_UPD_ENABLE == 1)
...
void btm_ble_ltk_request(UINT16 handle, UINT8 rand[8], UINT16 ediv)
{
    ...
    if (p_dev_rec != NULL) {
        if (!smp_proc_ltk_request(p_dev_rec->bd_addr)) {
            btm_ble_ltk_request_reply(p_dev_rec->bd_addr, 0, dummy_stk);
        }
    }
...
void btm_ble_link_encrypted(BD_ADDR bd_addr, UINT8 encr_enable)
{
    ...
    if (p_dev_rec->p_callback && enc_cback) {
        if (encr_enable) {
            btm_sec_dev_rec_cback_event(p_dev_rec, BTM_SUCCESS, 1);
        } else if (p_dev_rec->role_master) {
            btm_sec_dev_rec_cback_event(p_dev_rec, BTM_ERR_PROCESSING, 1);
        }
...
#endif
    {
#if BTM_BLE_CONFORMANCE_TESTING == 1
        if (btm_cb.devcb.keep_rfu_in_auth_req) {
            ...
            btm_cb.devcb.keep_rfu_in_auth_req = 0;
        } else {
            /* default */
                        ...
                        tBLE_ADDR_TYPE addr_type, BOOLEAN addr_matched)
{
    ...
    */
    ...
    p_dev_rec->role_master = 0;
    if (role == HCI_ROLE_MASTER) {
        p_dev_rec->role_master = 1;
    }

#if (defined BLE_PRIVACY_SPT && BLE_PRIVACY_SPT == 1)
    if (!addr_matched) {
...
void btm_ble_conn_complete(UINT8 *p, UINT16 evt_len, BOOLEAN enhanced)
{
    ...
    if (status == 0) {
        ...
        match = btm_identity_addr_to_random_pseudo (bda, &bda_type, 1);
...
#if (defined BLE_PRIVACY_SPT && BLE_PRIVACY_SPT == 1)
            btm_ble_disable_resolving_list(BTM_BLE_RL_INIT, 1);
#endif
        } else {
#if (defined BLE_PRIVACY_SPT && BLE_PRIVACY_SPT == 1)
            btm_cb.ble_ctr_cb.inq_var.adv_mode  = BTM_BLE_ADV_DISABLE;
            btm_ble_disable_resolving_list(BTM_BLE_RL_ADV, 1);
#endif
        }
    ...
    if (p_dev_rec != NULL) {
        ...
        case SMP_COMPLT_EVT:
            ...
            if (event == SMP_COMPLT_EVT) {
                ...
                btm_sec_dev_rec_cback_event(p_dev_rec, res, 1);
            }
            break;
...
**                             be stored.
**
** Returns          1 if signing sucessul, otherwise 0.
**
*******************************************************************************/
#if (SMP_INCLUDED == 1)
    ...
    if (p_rec == NULL) {
        ...
        if ((p_buf = (UINT8 *)osi_malloc((UINT16)(len + 4))) != NULL) {
            ...
            if ((ret = aes_cipher_msg_auth_code(p_rec->ble.keys.lcsrk, p_buf, (UINT16)(len + 4),
                                                BTM_CMAC_TLEN_SIZE, p_mac)) == 1) {
                btm_ble_increment_sign_ctr(bd_addr, 1);
            }
...
** Returns          1 if signature verified correctly; otherwise 0.
**
*******************************************************************************/
    ...
    if (p_rec == NULL || (p_rec && !(p_rec->ble.key_type & BTM_LE_KEY_PCSRK))) {
        ...
        if (aes_cipher_msg_auth_code(p_rec->ble.keys.pcsrk, p_orig, len, BTM_CMAC_TLEN_SIZE, p_mac)) {
            if (memcmp(p_mac, p_comp, BTM_CMAC_TLEN_SIZE) == 0) {
                btm_ble_increment_sign_ctr(bd_addr, 0);
                verified = 1;
            }
        }
    }
...
**
** Returns          BOOLEAN 1 if LE device is found, 0 otherwise.
**
*******************************************************************************/
...
**
** Returns          BOOLEAN 1 if security procedure is running, 0 otherwise.
**
*******************************************************************************/
...
#define L2CAP_LE_COC_INCLUDED   0
#endif
#if ((BLE_INCLUDED == 1) && (L2CAP_LE_COC_INCLUDED == 1))
    tBTM_SEC_DEV_REC *p_dev_rec = btm_find_dev (bd_addr);
...
**
** Returns          1 if the information is geted, else 0
**
*******************************************************************************/
