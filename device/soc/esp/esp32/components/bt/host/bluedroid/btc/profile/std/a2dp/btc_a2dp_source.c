...
{
    return a2dp_source_local_param.btc_aa_src_cb.is_tx_timer == 1;
}
...
void btc_a2dp_source_encoder_update(void)
{
    ...
    if (bta_av_co_get_remote_bitpool_pref(&pref_min, &pref_max) == 1) {
    ...
    while (nb_frame) {
        ...
        do {
            ...
            if (btc_media_aa_read_feeding()) {
                ...
                if ( a2dp_source_local_param.btc_aa_src_cb.is_tx_timer == 0 ) {
                    osi_free(p_buf);
                    return;
                }
            }
...
static void btc_a2dp_source_handle_timer(UNUSED_ATTR void *context)
{
    ...
    if (a2dp_source_local_param.btc_aa_src_cb.is_tx_timer == 1) {
...
static void btc_a2dp_source_aa_stop_tx(void)
{
    ...
    const bool send_ack = (a2dp_source_local_param.btc_aa_src_cb.is_tx_timer != 0);
    ...
    a2dp_source_local_param.btc_aa_src_cb.is_tx_timer = 0;
...
static void btc_a2dp_source_aa_start_tx(void)
{
    ...
    a2dp_source_local_param.btc_aa_src_cb.is_tx_timer = 1;
    a2dp_source_local_param.last_frame_us = 0;
...
static void btc_a2dp_source_thread_cleanup(UNUSED_ATTR void *context)
{
    btc_a2dp_control_set_datachnl_stat(0);
