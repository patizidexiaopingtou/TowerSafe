...
#if (BT_USE_TRACES == 1 && BT_TRACE_VERBOSE == 0)
/* needed for sprintf() */
#include <stdio.h>
#endif
...
#if (BT_USE_TRACES == 1 && SMP_INCLUDED == 1)
...
/* 1 - authenticated link key is possible */
...
**
** Returns          1 if registered OK, else 0
**
*******************************************************************************/
...
**
*******************************************************************************/
#if (SMP_INCLUDED == 1) || (CLASSIC_BT_INCLUDED == 1)
...
**
** Returns          BOOLEAN 1 or 0 is device found
**
*******************************************************************************/
...
**
** Parameters       allow_pairing - (1 or 0) whether or not the device
**                      allows pairing.
**                  connect_only_paired - (1 or 0) whether or not to
...
**
** Parameter        secure_connections_only_mode - (1 or 0) whether or not the device
**                  1 means that the device should treat Mode 4 Level 0 services as
**                  services of other levels. (Secure_connections_only_mode)
**                  0 means that the device should provide default treatment for
...
**
** Parameters:      is_originator - 1 if originating the connection, 0 if not
...
** Description      Register service security level with Security Manager
**
** Parameters:      conn_type   - 1 if originating the connection, 0 if not
**                  p_name      - Name of the service relevant only if
**                                authorization will show this name to user. ignored
**                                if BTM_SEC_SERVICE_NAME_LEN is 0.
**                  service_id  - service ID for the service passed to authorization callback
...
void BTM_PINCodeReply (BD_ADDR bd_addr, UINT8 res, UINT8 pin_len, UINT8 *p_pin, UINT32 trusted_mask[])
{
...
#endif  ///BT_USE_TRACES == 1 && SMP_INCLUDED == 1
    ...
    if ( (btm_cb.pairing_flags & BTM_PAIR_FLAGS_WE_STARTED_DD)
            ...
            &&  (btm_cb.security_mode_changed == 0) ) {
        /* This is start of the dedicated bonding if local device is 2.0 */
        btm_cb.pin_code_len = pin_len;
        p_dev_rec->pin_code_length = pin_len;
        memcpy (btm_cb.pin_code, p_pin, pin_len);

        btm_cb.security_mode_changed = 1;
#ifdef APPL_AUTH_WRITE_EXCEPTION
                                       ...
                                       UINT8 pin_len, UINT8 *p_pin, UINT32 trusted_mask[])
{
...
#if BTM_SEC_FORCE_RNR_FOR_DBOND == 1
    ...
    if (p && p->hci_handle != BTM_SEC_INVALID_HANDLE) {
        ...
        /* Mark lcb as bonding */
        l2cu_update_lcb_4_bonding (bd_addr, 1);
        return (BTM_CMD_STARTED);
    }
...
**                         transport    - 0 for BR/EDR link; 1 for LE link
**
*******************************************************************************/
    ...
    if ( (btm_cb.pairing_state != BTM_PAIR_STATE_IDLE)
        ...
        if (p_dev_rec->hci_handle != BTM_SEC_INVALID_HANDLE) {
            ...
            if (btm_cb.pairing_flags & BTM_PAIR_FLAGS_DISC_WHEN_DONE) {
                ...
                l2cu_update_lcb_4_bonding(bd_addr, 0);
            }
...
**                  transport  -    1 to encryption the link over LE transport
**                                  or 0 for BR/EDR transport
**
...
**
*******************************************************************************/
#if (BT_SSP_INCLUDED == 1 && SMP_INCLUDED == 1)
...
#endif  ///BT_SSP_INCLUDED == 1 && SMP_INCLUDED == 1
...
tBTM_STATUS BTM_ReadLocalOobData(void)
{
    ...
    if (btsnd_hcic_read_local_oob_data() == 0) {
        status = BTM_NO_RESOURCES;
    }
...
#endif  ///BTM_OOB_INCLUDED == 1 && SMP_INCLUDED == 1
...
**
** Returns          1 if BR/EDR Secure Connections are supported by both local
...
** Parameters:      bd_addr - address of the peer
**
** Returns          1 if BR/EDR Secure Connections are supported by the peer,
**                  else 0.
**
*******************************************************************************/
...
**
** Description      This function returns 1 if the existing link key
...
static void btm_sec_check_upgrade(tBTM_SEC_DEV_REC  *p_dev_rec, BOOLEAN is_originator)
{
    ...
    if (btm_sec_is_upgrade_possible (p_dev_rec, is_originator) == 1) {
...
** Parameters:      bd_addr       - Address of the peer device
**                  psm           - L2CAP PSM
**                  is_originator - 1 if protocol above L2CAP originates
**                                  connection
**                  p_callback    - Pointer to callback function called if
**                                  this function returns PENDING after required
                                      ...
                                      void *p_ref_data)
{
    ...
    BOOLEAN       chk_acp_auth_done = 0;
    BOOLEAN is_originator;
    BOOLEAN     transport = 0; /* should check PSM range in LE connection oriented L2CAP connection */
    ...
    if ( (p_dev_rec->p_callback) || (btm_cb.pairing_state != BTM_PAIR_STATE_IDLE) ) {
        ...
        if ((btm_cb.security_mode == BTM_SEC_MODE_UNDEFINED ||
                ...
                (BTM_SM4_KNOWN == p_dev_rec->sm4) || (BTM_SEC_IS_SM4(p_dev_rec->sm4) &&
                        (btm_sec_is_upgrade_possible(p_dev_rec, is_originator) == 0))) {
...
#if BTM_ALLOW_CONN_IF_NONDISCOVER == 0
...
static void btm_sec_bond_cancel_complete (void)
{
    ...
    if ((btm_cb.pairing_flags & BTM_PAIR_FLAGS_DISC_WHEN_DONE) ||
         ...
         * In theory, the code that handles is_pairing_device/1 should clean out security related code.
...
void btm_sec_dev_reset (void)
{
    if (controller_get_interface()->supports_simple_pairing()) {
        ...
        BTM_SetSecurityLevel(0, "RFC_MUX\n", BTM_SEC_SERVICE_RFC_MUX,
...
static tBTM_STATUS btm_sec_dd_create_conn (tBTM_SEC_DEV_REC *p_dev_rec)
{
    ...
    if (!p_lcb && (p_lcb = l2cu_allocate_lcb (p_dev_rec->bd_addr, 1, BT_TRANSPORT_BR_EDR)) == NULL) {
    ...
    if (l2cu_create_conn(p_lcb, BT_TRANSPORT_BR_EDR) == 0) {
...
void btm_sec_rmt_name_request_complete (UINT8 *p_bd_addr, UINT8 *p_bd_name, UINT8 status)
{
    ...
    */
    ...
    if ( (btm_cb.pairing_state == BTM_PAIR_STATE_WAIT_LOCAL_PIN) && p_bd_addr
        ...
        if (((btm_cb.pairing_flags & BTM_PAIR_FLAGS_WE_STARTED_DD) == 0) &&
            ...
            (*btm_cb.api.p_pin_callback) (p_dev_rec->bd_addr, p_dev_rec->dev_class, p_bd_name,
                                          (p_dev_rec->p_cur_service == NULL) ? 0
    ...
    if (status != HCI_SUCCESS) {
        btm_sec_dev_rec_cback_event  (p_dev_rec, BTM_ERR_PROCESSING, 0);
        return;
    }
    ...
    btm_sec_dev_rec_cback_event  (p_dev_rec, status, 0);
}
#endif  ///SMP_INCLUDED == 1
...
void btm_io_capabilities_req (UINT8 *p)
{
    ...
    if ((btm_cb.security_mode == BTM_SEC_MODE_SC) && (p_dev_rec->num_read_pages == 0)) {
        ...
        p_dev_rec->remote_features_needed = 1;
        return;
    }
...
void btm_io_capabilities_rsp (UINT8 *p)
{
    ...
    /* coverity[uninit_use_in_call]
    Event uninit_use_in_call: Using uninitialized element of array "evt_data.bd_addr" in call to function "memcmp"
    0-POSITIVE error from Coverity test-tool. evt_data.bd_addr is set at the beginning with:     STREAM_TO_BDADDR (evt_data.bd_addr, p);
    */
    if (!memcmp (evt_data.bd_addr, btm_cb.connecting_bda, BD_ADDR_LEN)) {
        memcpy (p_dev_rec->dev_class, btm_cb.connecting_dc, DEV_CLASS_LEN);
    }
...
// #endif
        ...
        } else if ( (event == BTM_SP_CFM_REQ_EVT) && (evt_data.cfm_req.just_works == 1) ) {
    ...
    if (BTM_SP_CFM_REQ_EVT == event) {
        btsnd_hcic_user_conf_reply (p_bda, 0);
...
void btm_rem_oob_req (UINT8 *p)
{
    ...
    if ( (NULL != (p_dev_rec = btm_find_dev (p_bda))) &&
        ...
        if ((*btm_cb.api.p_sp_callback) (BTM_SP_RMT_OOB_EVT, (tBTM_SP_EVT_DATA *)&evt_data) == BTM_NOT_AUTHORIZED) {
            BTM_RemoteOobDataReply(1, p_bda, c, r);
        }
        return;
    }
    ...
    if ((status == HCI_SUCCESS) && encr_enable && (p_dev_rec->hci_handle == handle)) {
        if (p_dev_rec->new_encryption_key_is_p256) {
            if (btm_sec_use_smp_br_chnl(p_dev_rec) &&
                ...
                p_dev_rec->new_encryption_key_is_p256 = 0;
    ...
    */
...
#if BTM_DISC_DURING_RS == 1
                    p_dev_rec->rs_disc_pending   = BTM_SEC_RS_NOT_PENDING;     /* reset flag */
#endif
                    return;
                } else {
                    l2cu_update_lcb_4_bonding(p_dev_rec->bd_addr, 1);
                }
            }
    ...
    if (status != HCI_SUCCESS) {
        ...
        if (status == HCI_ERR_CONNECTION_TOUT || status == HCI_ERR_LMP_RESPONSE_TIMEOUT  ||
            ...
            btm_sec_dev_rec_cback_event (p_dev_rec, BTM_DEVICE_TIMEOUT, 0);
        } else {
            btm_sec_dev_rec_cback_event (p_dev_rec, BTM_ERR_PROCESSING, 0);
        }
...
#if (!defined(BTM_BYPASS_EXTRA_ACL_SETUP) || BTM_BYPASS_EXTRA_ACL_SETUP == 0)
    ...
    if (!(p_dev_rec->sec_flags & BTM_SEC_NAME_KNOWN) || p_dev_rec->is_originator) {
        if ((res = btm_sec_execute_procedure (p_dev_rec)) != BTM_CMD_STARTED) {
            btm_sec_dev_rec_cback_event (p_dev_rec, res, 0);
        }
    }
    return;
}
...
void btm_sec_link_key_notification (UINT8 *p_bda, UINT8 *p_link_key, UINT8 key_type)
{
    ...
    BOOLEAN         we_are_bonding = 0;
    BOOLEAN         ltk_derived_lk  = 0;
    ...
    if (ltk_derived_lk) {
        ...
        if ((p_dev_rec->link_key_type == BTM_LKEY_TYPE_UNAUTH_COMB_P_256) ||
                (p_dev_rec->link_key_type == BTM_LKEY_TYPE_AUTH_COMB_P_256)) {
            p_dev_rec->new_encryption_key_is_p256 = 1;
...
static void btm_sec_pairing_timeout (TIMER_LIST_ENT *p_tle)
{
    ...
    /* Coverity: 0-POSITIVE error from Coverity tool. Please do NOT remove following comment. */
    ...
    */
    ...
    case BTM_PAIR_STATE_WAIT_NUMERIC_CONFIRM:
        btsnd_hcic_user_conf_reply (p_cb->pairing_bda, 0);
        /* btm_sec_change_pairing_state (BTM_PAIR_STATE_IDLE); */
        break;
...
static tBTM_STATUS btm_sec_execute_procedure (tBTM_SEC_DEV_REC *p_dev_rec)
{
    ...
    if ((((!(p_dev_rec->sec_flags & BTM_SEC_AUTHENTICATED))
        ...
        if ( !p_dev_rec->is_originator && (p_dev_rec->is_ucd == 1 )) {
    ...
    if (!(p_dev_rec->sec_flags & BTM_SEC_AUTHORIZED)
        ...
        BTM_TRACE_EVENT ("service id:%d, is trusted:%d\n",
                         ...
                         (BTM_SEC_IS_SERVICE_TRUSTED(p_dev_rec->trusted_mask,
                                 p_dev_rec->p_cur_service->service_id)));
        if ((btm_sec_are_all_trusted(p_dev_rec->trusted_mask) == 0) &&
                (p_dev_rec->p_cur_service->service_id < BTM_SEC_MAX_SERVICES) &&
                (BTM_SEC_IS_SERVICE_TRUSTED(p_dev_rec->trusted_mask,
                                            p_dev_rec->p_cur_service->service_id) == 0)) {
...
static BOOLEAN btm_sec_start_encryption (tBTM_SEC_DEV_REC *p_dev_rec)
{
    if (!btsnd_hcic_set_conn_encrypt (p_dev_rec->hci_handle, 1)) {
        return (0);
    }
...
**
** Returns          1 if all are trusted, otherwise 0
**
*******************************************************************************/
BOOLEAN btm_sec_are_all_trusted(UINT32 p_mask[])
{
...
static void btm_sec_collision_timeout (TIMER_LIST_ENT *p_tle)
{
    ...
    if (status != BTM_CMD_STARTED) {
        /* There is no next procedure or start of procedure failed, notify the waiting layer */
        btm_sec_dev_rec_cback_event (btm_cb.p_collided_dev_rec, status, 0);
    }
}
...
static void btm_sec_change_pairing_state (tBTM_PAIRING_STATE new_state)
{
    ...
    if (new_state == BTM_PAIR_STATE_IDLE) {
        ...
        l2cu_update_lcb_4_bonding (btm_cb.pairing_bda, 0);
        ...
        if (old_state == BTM_PAIR_STATE_IDLE) {
            l2cu_update_lcb_4_bonding (btm_cb.pairing_bda, 1);
        }
...
static BOOLEAN btm_sec_check_prefetch_pin (tBTM_SEC_DEV_REC  *p_dev_rec)
{
    ...
    if ((major == BTM_COD_MAJOR_AUDIO)
        ...
        if (btm_cb.security_mode_changed == 0) {
            btm_cb.security_mode_changed = 1;
#ifdef APPL_AUTH_WRITE_EXCEPTION
            if (!(APPL_AUTH_WRITE_EXCEPTION)(p_dev_rec->bd_addr))
#endif
        ...
        if (btm_cb.pin_code_len) {
            ...
            if (btm_cb.api.p_pin_callback && ((btm_cb.pairing_flags & BTM_PAIR_FLAGS_PIN_REQD) == 0)) {
                ...
                (btm_cb.api.p_pin_callback) (p_dev_rec->bd_addr, p_dev_rec->dev_class,
                                             p_dev_rec->sec_bd_name, (p_dev_rec->p_cur_service == NULL) ? 0
                                             : (p_dev_rec->p_cur_service->security_flags
                                                & BTM_SEC_IN_MIN_16_DIGIT_PIN));
            }
        }
...
void btm_sec_set_peer_sec_caps(tACL_CONN *p_acl_cb, tBTM_SEC_DEV_REC *p_dev_rec)
{
    ...
    if ((btm_cb.security_mode == BTM_SEC_MODE_SP ||
        ...
        p_dev_rec->sm4 = BTM_SM4_KNOWN;
        p_dev_rec->remote_supports_secure_connections = 0;
    }
    ...
    if (p_dev_rec->remote_features_needed) {
        ...
        p_dev_rec->remote_features_needed = 0;
    }
}
...
**
** Returns          1 if the service is security mode 4 level 0 service
**
*******************************************************************************/
static BOOLEAN btm_sec_is_serv_level0(UINT16 psm)
{
...
BOOLEAN btm_sec_is_a_bonded_dev (BD_ADDR bda)
{
    ...
    if (p_dev_rec &&
#if (SMP_INCLUDED == 1 && BLE_INCLUDED == 1)
...
**
** Returns          1 - dev is a dual mode
**
*******************************************************************************/
BOOLEAN btm_sec_is_le_capable_dev (BD_ADDR bda)
{
    BOOLEAN le_capable = 0;
...
**
** Returns          1 - found a bonded device
**
*******************************************************************************/
#if (BLE_INCLUDED == 1)
...
**
** Returns          1 - if SMP BR connection can be used (the link key is
...
**
** Returns          1 - if the device is master.
**
*******************************************************************************/
static BOOLEAN btm_sec_is_master(tBTM_SEC_DEV_REC *p_dev_rec)
{
...
**
** Returns          1 if aunthentication command sent successfully
**
*******************************************************************************/
