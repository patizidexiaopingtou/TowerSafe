...
BOOLEAN l2c_link_hci_conn_req (BD_ADDR bd_addr)
{
    ...
    if (!p_lcb) {
        p_lcb = l2cu_allocate_lcb (bd_addr, 0, BT_TRANSPORT_BR_EDR);
        if (!p_lcb) {
...
**                  event is received.
**
** Returns          1 if the link is known about, else 0
**
*******************************************************************************/
BOOLEAN l2c_link_hci_disc_comp (UINT16 handle, UINT8 reason)
{
    ...
    if (!p_lcb) {
        ...
        if (p_lcb->transport == BT_TRANSPORT_LE) {
            btm_ble_update_link_topology_mask(p_lcb->link_role, 0);
        }
#endif
#if (CLASSIC_BT_INCLUDED == 1)
        ...
        if (p_lcb->ccb_queue.p_first_ccb != NULL || p_lcb->p_pending_ccb) {
            ...
            if (l2cu_create_conn(p_lcb, transport)) {
                lcb_is_free = 0;    /* still using this lcb */
            }
        }

        p_lcb->p_pending_ccb = NULL;
#if (BLE_INCLUDED == 1 && GATTC_CONNECT_RETRY_EN == 1)
    ...
    if (bd_addr) {
        ...
        if (p_lcb) {
            ...
            if (hci_status == HCI_SUCCESS) {
                l2cu_set_acl_priority(bd_addr, p_lcb->acl_priority, 1);
            }
        }
    }
...
**
** Returns          1 if link is going to be active from park
**                  0 if nothing to send or not in park mode
**
*******************************************************************************/
...
#endif /* L2CAP_WAKE_PARKED_LINK == 1) */
...
#if BLE_INCLUDED == 1
            if (p_lcb->transport == BT_TRANSPORT_LE) {
                l2cb.ble_check_round_robin = 1;
            } else
#endif
            {
                l2cb.check_round_robin = 1;
            }
        }
    }
...
static BOOLEAN l2c_link_send_to_lower (tL2C_LCB *p_lcb, BT_HDR *p_buf)
{
    ...
    if ((p_buf->len <= controller->get_acl_packet_size_classic()
        ...
        if (p_lcb->link_xmit_quota == 0) {
            num_segs = 1;
            p_lcb->partial_segment_being_sent = 1;
        } else {
...
#if (L2CAP_HCI_FLOW_CONTROL_DEBUG == 1)
#if (BLE_INCLUDED == 1)
    if (p_lcb->transport == BT_TRANSPORT_LE) {
...
#if (defined(HCILP_INCLUDED) && HCILP_INCLUDED == 1)
...
void l2c_link_segments_xmitted (BT_HDR *p_msg)
{
    ...
    if (p_lcb->link_state == LST_CONNECTED) {
        ...
        p_lcb->partial_segment_being_sent = 0;
