...
typedef struct {
    ...
    BOOLEAN  peer_fc;       /* 1 if flow control is set based on peer's request */
    BOOLEAN  user_fc;       /* 1 if flow control is set based on user's request  */
...
#define RFCOMM_CTRL_BREAK_IN_SEQ    1
    ...
    BOOLEAN fc;                 /* 1 when the device is unable to accept frames */
} tPORT_CTRL;
...
typedef struct {
    ...
    UINT8     is_initiator;   /* 1 if this side sends SABME (dlci=0) */
    BOOLEAN   local_cfg_sent;
    BOOLEAN   peer_cfg_rcvd;
    BOOLEAN   restart_required; /* 1 if has to restart channel after disc */
    BOOLEAN   peer_ready;      /* True if other side can accept frames */
    UINT8     flow;            /* flow control mechanism for this mux */
    BOOLEAN   l2cap_congested; /* 1 if L2CAP is congested */
    BOOLEAN   is_disc_initiator; /* 1 if initiated disc of port */
...
#define PORT_STATE_CLOSING  3
    ...
    BOOLEAN is_server;      /* 1 if the server application */
...
#define PORT_CTRL_IND_RESPONDED     0x08
    ...
    BOOLEAN     keep_port_handle;           /* 1 if port is not deallocated when closing */
    /* it is set to 1 for server when allocating port */
