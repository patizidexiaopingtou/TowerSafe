        ...
        if (callback_rc == SMP_SUCCESS) {
            ...
            case SMP_IO_CAP_REQ_EVT:
                ...
                p_cb->secure_connections_only_mode_required =
                    (btm_cb.security_mode == BTM_SEC_MODE_SC) ? 1 : 0;
...
void smp_send_pair_fail(tSMP_CB *p_cb, tSMP_INT_DATA *p_data)
{
    ...
    if (p_cb->status <= SMP_MAX_FAIL_RSN_PER_SPEC && p_cb->status != SMP_SUCCESS) {
        ...
        p_cb->wait_for_authorization_complete = 1;
    }
}
...
void smp_send_enc_info(tSMP_CB *p_cb, tSMP_INT_DATA *p_data)
{
    ...
    smp_update_key_mask (p_cb, SMP_SEC_KEY_TYPE_ENC, 0);
...
{
    SMP_TRACE_DEBUG("%s\n", __func__);
    smp_update_key_mask (p_cb, SMP_SEC_KEY_TYPE_ID, 0);
...
void smp_send_csrk_info(tSMP_CB *p_cb, tSMP_INT_DATA *p_data)
{
    ...
    SMP_TRACE_DEBUG("%s\n", __func__);
    smp_update_key_mask (p_cb, SMP_SEC_KEY_TYPE_CSRK, 0);
    ...
    if (smp_send_cmd(SMP_OPCODE_SIGN_INFO, p_cb)) {
        ...
        btm_sec_save_le_key(p_cb->pairing_bda, BTM_LE_KEY_LCSRK, (tBTM_LE_KEY_VALUE *)&key, 1);
    }
...
void smp_send_ltk_reply(tSMP_CB *p_cb, tSMP_INT_DATA *p_data)
{
    ...
    btm_ble_ltk_request_reply(p_cb->pairing_bda, 1, p_data->key.p_data);
}
...
void smp_proc_master_id(tSMP_CB *p_cb, tSMP_INT_DATA *p_data)
{
    ...
    smp_update_key_mask (p_cb, SMP_SEC_KEY_TYPE_ENC, 1);
...
void smp_proc_id_addr(tSMP_CB *p_cb, tSMP_INT_DATA *p_data)
{
    ...
    SMP_TRACE_DEBUG("%s", __func__);
    smp_update_key_mask (p_cb, SMP_SEC_KEY_TYPE_ID, 1);
...
void smp_proc_srk_info(tSMP_CB *p_cb, tSMP_INT_DATA *p_data)
{
    ...
    SMP_TRACE_DEBUG("%s", __func__);
    smp_update_key_mask (p_cb, SMP_SEC_KEY_TYPE_CSRK, 1);
...
void smp_start_enc(tSMP_CB *p_cb, tSMP_INT_DATA *p_data)
{
    ...
    SMP_TRACE_DEBUG("%s\n", __func__);
    if (p_data != NULL) {
        cmd = btm_ble_start_encrypt(p_cb->pairing_bda, 1, p_data->key.p_data);
    } else {
        cmd = btm_ble_start_encrypt(p_cb->pairing_bda, 0, NULL);
    }
...
void smp_fast_conn_param(tSMP_CB *p_cb, tSMP_INT_DATA *p_data)
{
    ...
    if(p_cb->role == BTM_ROLE_MASTER) {
        L2CA_EnableUpdateBleConnParams(p_cb->pairing_bda, 0);
    }
#endif
...
void smp_set_derive_link_key(tSMP_CB *p_cb, tSMP_INT_DATA *p_data)
{
    SMP_TRACE_DEBUG ("%s\n", __func__);
    p_cb->derive_lk = 1;
    smp_update_key_mask (p_cb, SMP_SEC_KEY_TYPE_LK, 0);
    smp_key_distribution(p_cb, NULL);
}
