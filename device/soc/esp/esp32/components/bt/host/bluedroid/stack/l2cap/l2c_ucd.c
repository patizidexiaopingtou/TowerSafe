...
static BOOLEAN l2c_ucd_connect ( BD_ADDR rem_bda )
{
    ...
    if ((p_lcb = l2cu_find_lcb_by_bd_addr (rem_bda, BT_TRANSPORT_BR_EDR)) == NULL) {
        ...
        if ( ((p_lcb = l2cu_allocate_lcb (rem_bda, 0, BT_TRANSPORT_BR_EDR)) == NULL)
...
**
**  Return          1 if any pending UCD info request
**
*******************************************************************************/
...
**
**  Return          1 if any UCD packet for security
**
*******************************************************************************/
...
**
**  Return          1 if UCD reception is registered
**
*******************************************************************************/
...
**
**  Return          1 if the event is consumed by UCD
**                  0 if the event needs to be processed by main state machine
**
*******************************************************************************/
BOOLEAN l2c_ucd_process_event(tL2C_CCB *p_ccb, UINT16 event, void *p_data)
{
    /* if the event is not processed by this function, this variable will be set to 0 */
    BOOLEAN done = 1;
    ...
    case CST_CLOSED:
        ...
        default:
            done = 0;   /* main state machine continues to process event */
            break;
        }
        break;
