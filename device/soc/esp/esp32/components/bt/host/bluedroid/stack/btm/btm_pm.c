...
/* Usage:  (ptr_features[ offset ] & mask )?1:0 */
...
static tBTM_STATUS btm_pm_snd_md_req(UINT8 pm_id, UINT16 link_hdl, tBTM_PM_PWR_MD *p_mode)
{
    ...
    if ( mode != BTM_PM_MD_ACTIVE && p_cb->state != BTM_PM_MD_ACTIVE) {
        p_cb->chg_ind = 1;    /* needs to wake, then sleep */
    }

    if (p_cb->chg_ind == 1) { /* needs to wake first */
...
**
** Returns          1, if in active or sniff mode
**
*******************************************************************************/
...
**
** Returns          1, if in paging, inquiry or connecting mode
**
*******************************************************************************/
...
{
    if (1 == btm_pm_device_in_active_or_sniff_mode()) {
        return BTM_CONTRL_ACTIVE;
    } else if (1 == btm_pm_device_in_scan_state()) {
