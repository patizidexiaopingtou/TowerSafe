...
**
** Returns      1 if event was processed, 0 otherwise
**
*******************************************************************************/
        ...
        if (event == BTC_AV_CONNECT_REQ_EVT) {
            ...
            if (g_av_with_rc) {
                BTA_AvOpen(btc_av_cb.peer_bda.address, btc_av_cb.bta_handle,
                        1, BTA_SEC_AUTHENTICATE, ((btc_av_connect_req_t *)p_data)->uuid);
            } else {
                BTA_AvOpen(btc_av_cb.peer_bda.address, btc_av_cb.bta_handle,
                        0, BTA_SEC_AUTHENTICATE, ((btc_av_connect_req_t *)p_data)->uuid);
            }
            ...
            if (g_av_with_rc) {
                BTA_AvOpen(btc_av_cb.peer_bda.address, btc_av_cb.bta_handle,
                        1, BTA_SEC_AUTHENTICATE, uuid);
            } else {
                BTA_AvOpen(btc_av_cb.peer_bda.address, btc_av_cb.bta_handle,
                       0, BTA_SEC_AUTHENTICATE, uuid);
            }
        }
...
static BOOLEAN btc_av_state_closing_handler(btc_sm_event_t event, void *p_data)
{
    ...
    case BTC_SM_ENTER_EVT:
        ...
        if (btc_av_cb.peer_sep == AVDT_TSEP_SNK) {
            ...
            btc_a2dp_source_set_tx_flush(1);
            /* wait for audioflinger to stop a2dp */
        }
#endif /* BTC_AV_SRC_INCLUDED */
#if BTC_AV_SINK_INCLUDED
        if (btc_av_cb.peer_sep == AVDT_TSEP_SRC) {
            btc_a2dp_sink_set_rx_flush(1);
        }
#endif /* BTC_AV_SINK_INCLUDED */
        break;
...
static BOOLEAN btc_av_state_opened_handler(btc_sm_event_t event, void *p_data)
{
    ...
    case BTA_AV_START_EVT: {
        ...
        if ((p_av->start.status == BTA_SUCCESS) && (p_av->start.suspending == 1)) {
            return 1;
        }
#if BTC_AV_SRC_INCLUDED
...
#if BTC_AV_SINK_INCLUDED
        if (btc_av_cb.peer_sep == AVDT_TSEP_SRC) {
            btc_a2dp_sink_set_rx_flush(0); /*  remove flush state, ready for streaming*/
        }
#endif /* BTC_AV_SINK_INCLUDED */
#if BTC_AV_SRC_INCLUDED
    ...
    case BTA_AV_SUSPEND_EVT:
        ...
        if (p_av->suspend.status != BTA_AV_SUCCESS) {
            ...
            if (btc_av_cb.peer_sep == AVDT_TSEP_SNK) {
                ...
                btc_a2dp_source_set_tx_flush(0);
            }
#endif /* BTC_AV_SRC_INCLUDED */
            return 0;
        }

        if (p_av->suspend.initiator != 1) {
...
static void bte_av_media_callback(tBTA_AV_EVT event, tBTA_AV_MEDIA *p_data)
{
    ...
    if (event == BTA_AV_MEDIA_SINK_CFG_EVT) {
        ...
        a2d_status = A2D_ParsSbcInfo(&sbc_cie, (UINT8 *)p_data, 0);
...
**
** Returns          1 if remote device is capable of EDR
**
*******************************************************************************/
