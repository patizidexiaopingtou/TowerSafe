...
void rfc_send_sabme (tRFC_MCB *p_mcb, UINT8 dlci)
{
    BT_HDR  *p_buf;
    UINT8   *p_data;
    UINT8   cr = RFCOMM_CR(p_mcb->is_initiator, 1);
...
void rfc_send_ua (tRFC_MCB *p_mcb, UINT8 dlci)
{
    BT_HDR  *p_buf;
    UINT8   *p_data;
    UINT8   cr = RFCOMM_CR(p_mcb->is_initiator, 0);
...
void rfc_send_nsc (tRFC_MCB *p_mcb)
{
    ...
    *p_data++ = RFCOMM_EA | RFCOMM_I_CR(0) | RFCOMM_MX_NSC;
...
void rfc_process_mx_message (tRFC_MCB *p_mcb, BT_HDR *p_buf)
{
    ...
    case RFCOMM_MX_MSC:
        ...
        if (mx_len == RFCOMM_MX_MSC_LEN_WITH_BREAK) {
            ...
            p_rx_frame->u.msc.break_present  = 0;
