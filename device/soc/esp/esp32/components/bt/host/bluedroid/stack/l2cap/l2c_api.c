...
UINT16 L2CA_ErtmConnectReq (UINT16 psm, BD_ADDR p_bd_addr, tL2CAP_ERTM_INFO *p_ertm_info)
{
    ...
    if ((p_lcb = l2cu_find_lcb_by_bd_addr (p_bd_addr, BT_TRANSPORT_BR_EDR)) == NULL) {
        ...
        if ( ((p_lcb = l2cu_allocate_lcb (p_bd_addr, 0, BT_TRANSPORT_BR_EDR)) == NULL)
                /* currently use BR/EDR for ERTM mode l2cap connection */
                ||  (l2cu_create_conn(p_lcb, BT_TRANSPORT_BR_EDR) == 0) ) {
...
**
** Returns          1 if configuration sent, else 0
**
*******************************************************************************/
...
**                  response.
**
** Returns          1 if configuration response sent, else 0
**
*******************************************************************************/
BOOLEAN L2CA_ConfigRsp (UINT16 cid, tL2CAP_CFG_INFO *p_cfg)
{
    ...
    if ( (p_cfg->result == L2CAP_CFG_OK) || (p_cfg->result == L2CAP_CFG_PENDING) ) {
        ...
        p_cfg->fcr_present = 0; /* FCR options already negotiated before this point */

        /* Clear out any cached options that are being returned as an error (excluding FCR) */
        if (p_cfg->mtu_present) {
            p_ccb->peer_cfg.mtu_present = 0;
        }
        if (p_cfg->flush_to_present) {
            p_ccb->peer_cfg.flush_to_present = 0;
        }
        if (p_cfg->qos_present) {
            p_ccb->peer_cfg.qos_present = 0;
        }
...
**
** Returns          1 if disconnect sent, else 0
**
*******************************************************************************/
...
**
** Returns          1 if echo request sent, else 0.
**
*******************************************************************************/
...
UINT8 L2CA_SetDesireRole (UINT8 new_role)
{
    ...
    if (L2CAP_ROLE_CHECK_SWITCH != (L2CAP_ROLE_CHECK_SWITCH & new_role)) {
        ...
        if (new_role & L2CAP_ROLE_ALLOW_SWITCH) {
            l2cb.disallow_switch = 0;
        }
        if (new_role & L2CAP_ROLE_DISALLOW_SWITCH) {
            l2cb.disallow_switch = 1;
        }
    }
...
BOOLEAN L2CA_SetAclPriority (BD_ADDR bd_addr, UINT8 priority)
{
    ...
    return (l2cu_set_acl_priority(bd_addr, priority, 0));
}
...
**
**                  data_enabled - 1 data flows, 0 data is stopped
**
** Returns          1 if valid channel, else 0
**
*******************************************************************************/
...
**
**  Return value:    1 if peer is connected
**
*******************************************************************************/
...
**                   BD address of the peer
**
**  Return value:    1 if found lcb for the given handle, 0 otherwise
**
*******************************************************************************/
...
UINT16 L2CA_ConnectLECocReq(UINT16 psm, BD_ADDR p_bd_addr, tL2CAP_LE_CFG_INFO *p_cfg)
{
    ...
    {
        /* No link. Get an LCB and start link establishment */
        p_lcb = l2cu_allocate_lcb(p_bd_addr, 0, BT_TRANSPORT_LE);
        if ((p_lcb == NULL)
             /* currently use BR/EDR for ERTM mode l2cap connection */
         || (l2cu_create_conn(p_lcb, BT_TRANSPORT_LE) == 0) )
        {
...
**                  BD Address type
**
**  Return value:   1 if connection started
**
*******************************************************************************/
BOOLEAN L2CA_ConnectFixedChnl (UINT16 fixed_cid, BD_ADDR rem_bda, tBLE_ADDR_TYPE bd_addr_type, BOOLEAN is_aux)
{
    ...
    if ((p_lcb = l2cu_find_lcb_by_bd_addr (rem_bda, transport)) != NULL) {
        ...
        (fixed_cid, p_lcb->remote_bd_addr, 1, 0, p_lcb->transport);
#else
        (*l2cb.fixed_reg[fixed_cid - L2CAP_FIRST_FIXED_CHNL].pL2CA_FixedConn_Cb)
        (fixed_cid, p_lcb->remote_bd_addr, 1, 0, BT_TRANSPORT_BR_EDR);
#endif
        return 1;
    }

    // No link. Get an LCB and start link establishment
    if ((p_lcb = l2cu_allocate_lcb (rem_bda, 0, transport)) == NULL) {
...
**
**  Return value:   1 if channel removed
**
*******************************************************************************/
...
**
** Returns          L2CAP_DW_SUCCESS, if data accepted, else 0
    ...
    if (p_ccb->peer_cfg.fcr.mode != L2CAP_FCR_ERTM_MODE) {
        ...
        if (num_to_flush != L2CAP_FLUSH_CHANS_GET) {
            ...
            if ( (HCI_NON_FLUSHABLE_PB_SUPPORTED(BTM_ReadLocalFeatures ()))
                ...
                if ( l2cb.is_flush_active == 0 ) {
                    l2cb.is_flush_active = 1;
