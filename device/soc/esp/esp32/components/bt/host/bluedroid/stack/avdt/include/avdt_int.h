...
#if ((AVDT_MULTIPLEXING == 1) && (AVDT_REPORTING == 1))
...
#define AVDT_AD_FAILED       L2CAP_DW_FAILED        /* 0 */
#define AVDT_AD_SUCCESS      L2CAP_DW_SUCCESS       /* 1 */
...
typedef struct {
    ...
    BOOLEAN             ll_opened;      /* 1 if LL is opened */
    BOOLEAN             proc_busy;      /* 1 when a discover or get capabilities procedure in progress */
    ...
    BOOLEAN             reconn;         /* If 1, reinitiate connection after transitioning from CLOSING to IDLE state */
