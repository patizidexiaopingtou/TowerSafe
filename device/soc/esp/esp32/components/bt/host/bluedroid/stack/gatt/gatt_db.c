        ...
        UINT16       *p_cur_handle)
{
    ...
    tBT_UUID    attr_uuid;
#if (defined(BLE_DELAY_REQUEST_ENC) && (BLE_DELAY_REQUEST_ENC == 1))
    UINT8       flag;
#endif
    BOOLEAN need_rsp;
    ...
    if (p_db && p_db->p_attr_list) {
        ...
        while (p_attr && p_attr->handle <= e_handle) {
            ...
            if (p_attr->handle >= s_handle && gatt_uuid_compare(type, attr_uuid)) {
                ...
                status = read_attr_value ((void *)p_attr, 0, &p, 0, (UINT16)(*p_len - 2), &len, sec_flag, key_size);
                                   ...
                                   tBT_UUID service)
{
    ...
    BOOLEAN is_include_service_allowed = 1;
    // service declaration
...
**
** Returns          1 if allocation succeed, otherwise 0.
**
*******************************************************************************/
static BOOLEAN allocate_svc_db_buf(tGATT_SVC_DB *p_db)
{
        ...
        UINT16 handle, UINT16 offset, UINT32 trans_id, BOOLEAN need_rsp)
{
    ...
    if (trans_id == 0) {
        ...
        gatt_sr_update_cback_cnt(p_tcb, p_sreg->gatt_if, 1, 1);
    }
