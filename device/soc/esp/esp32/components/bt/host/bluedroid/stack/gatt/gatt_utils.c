...
**
** Description      Set the service changed flag to 1
**
** Returns        None
**
*******************************************************************************/
void gatt_set_srv_chg(void)
{
    ...
    for (const list_node_t *node = list_begin(list); node != list_end(list);
        ...
        if (!p_buf->srv_changed) {
            GATT_TRACE_DEBUG("set srv_changed to 1");
            p_buf->srv_changed = 1;
            tGATTS_SRV_CHG_REQ req;
...
**
** Returns       1 - yes this is the last attribute
**
*******************************************************************************/
...
void gatts_update_srv_list_elem(UINT8 i_sreg, UINT16 handle, BOOLEAN is_primary)
{
    ...
    gatt_cb.srv_list[i_sreg].in_use         = 1;
...
**
** Returns   BOOLEAN 1-if add is successful
**
*******************************************************************************/
...
**
** Returns   BOOLEAN 1-if remove is successful
**
*******************************************************************************/
...
**
** Returns          1 if two uuid match; 0 otherwise.
**
*******************************************************************************/
...
#if (SDP_INCLUDED == 1 && CLASSIC_BT_GATT_INCLUDED == 1)
/*******************************************************************************
**
...
#endif  ///SDP_INCLUDED == 1 && CLASSIC_BT_GATT_INCLUDED == 1
...
BOOLEAN gatt_cancel_open(tGATT_IF gatt_if, BD_ADDR bda)
{
    ...
    if (p_tcb) {
        if (gatt_get_ch_state(p_tcb) == GATT_CH_OPEN) {
            ...
            gatt_update_app_use_link_flag(gatt_if, p_tcb, 0, 0);
	...
	for(p_node = list_begin(gatt_cb.p_clcb_list); p_node; p_node = p_node_next) {
        ...
        for (i = 0; i < GATT_MAX_APPS; i ++) {
            ...
            if (p_reg->in_use && p_reg->app_cb.p_conn_cb) {
                ...
                (*p_reg->app_cb.p_conn_cb)(p_reg->gatt_if,  bda, conn_id, 0, reason, transport);
            }
        }
        gatt_tcb_free(p_tcb);
    } else {
...
**
** Returns          1 this is one of the background devices for the  application
**
*******************************************************************************/
...
**
** Returns          1 if device added to the list; 0 failed
**
*******************************************************************************/
        ...
        for (i = 0; i < GATT_MAX_APPS; i ++) {
            if (is_initator) {
                if (p_dev->gatt_if[i] == gatt_if) {
                    ...
                    if (i == 0) {
                        ret = BTM_BleUpdateBgConnDev(1, bd_addr);
                    } else {
                        ret = 1;
                    }
                    break;
                ...
                if (p_dev->listen_gif[i] == gatt_if) {
                    ...
                    if (i == 0) {
                        ...
                        ret = BTM_BleUpdateAdvWhitelist(1, bd_addr, 0, NULL);
                    } else {
                        ret = 1;
                    }
                    break;
...
BOOLEAN gatt_remove_bg_dev_for_app(tGATT_IF gatt_if, BD_ADDR bd_addr)
{
    ...
    status = gatt_update_auto_connect_dev(gatt_if, 0, bd_addr, 1);
    return status;
}
        ...
        if (is_initiator) {
            if (p_dev->gatt_if[i] == gatt_if) {
                ...
                if (p_dev->gatt_if[0] == 0) {
                    ret = BTM_BleUpdateBgConnDev(0, p_dev->remote_bda);
                } else {
                    ret = 1;
                }
            ...
            if (p_dev->listen_gif[i] == gatt_if) {
                ...
                if (p_dev->listen_gif[0] == 0) {
                    // To check, we do not support background connection, code will not be called here
                    ret = BTM_BleUpdateAdvWhitelist(0, p_dev->remote_bda, 0, NULL);
                } else {
                    ret = 1;
                }
                break;
            ...
            for (j = 0; j < GATT_MAX_APPS; j ++) {
                ...
                if (p_dev_list->gatt_if[j] == gatt_if) {
                    ...
                    if (p_dev_list->gatt_if[0] == 0) {
                        BTM_BleUpdateBgConnDev(0, p_dev_list->remote_bda);
                    }
                }
                ...
                if (p_dev_list->listen_gif[j] == gatt_if) {
                    ...
                    if (p_dev_list->listen_gif[0] == 0) {
                        // To check, we do not support background connection, code will not be called here
                        BTM_BleUpdateAdvWhitelist(0, p_dev_list->remote_bda, 0, NULL);
                    }
                }
            }
        }
...
**
** Returns          1 if connection started; 0 if connection start failure.
**
*******************************************************************************/
BOOLEAN gatt_update_auto_connect_dev (tGATT_IF gatt_if, BOOLEAN add, BD_ADDR bd_addr, BOOLEAN is_initator)
{
    ...
    if (add) {
        ...
        if (ret && p_tcb != NULL) {
            ...
            gatt_update_app_use_link_flag(gatt_if, p_tcb, 1, 1);
        }
    } else {
