...
#if (SDP_INCLUDED == 1 || SMP_INCLUDED == 1)
static char *bta_dm_get_remname(void);
#endif  ///SDP_INCLUDED == 1 || SMP_INCLUDED == 1
#if (SMP_INCLUDED == 1)
...
#if ((defined BLE_INCLUDED) && (BLE_INCLUDED == 1))
#if ((defined SMP_INCLUDED) && (SMP_INCLUDED == 1))
...
#endif  ///SMP_INCLUDED == 1
#if ((defined BTA_GATT_INCLUDED) &&  (BTA_GATT_INCLUDED == 1) && SDP_INCLUDED == 1)
#if (GATTC_INCLUDED == 1)
...
void bta_dm_enable(tBTA_DM_MSG *p_data)
{
    ...
    if ( bta_dm_cb.is_bta_dm_active == 1  ) {
...
#if ( BTA_EIR_CANNED_UUID_LIST != 1 )
    bta_sys_free_timer(&bta_dm_cb.app_ready_timer);
#endif
#if BTM_SSR_INCLUDED == 1
...
static void bta_dm_eir_cfg_init(void)
{
    ...
    p_bta_dm_eir_cfg->bta_dm_eir_included_uuid = 1;
    p_bta_dm_eir_cfg->bta_dm_eir_included_tx_power = 0;
...
static void bta_dm_sys_hw_cback( tBTA_SYS_HW_EVT status )
{
    ...
    if ( status == BTA_SYS_HW_OFF_EVT ) {
        ...
        bta_dm_cb.is_bta_dm_active = 0;
#if (defined BLE_INCLUDED && BLE_INCLUDED == 1)
#if (GATTC_INCLUDED == 1 && GATTC_CACHE_NVS == 1)
        ...
        * We need to revisit when this platform has more than one BLuetooth H/W chip */
        ...
        bta_dm_init_pm();
#endif /* #if (BTA_DM_PM_INCLUDED == 1) */

        bta_sys_policy_register((tBTA_SYS_CONN_CBACK *)bta_dm_policy_cback);

#if (BLE_INCLUDED == 1 && BTA_GATT_INCLUDED == 1 && SDP_INCLUDED == 1) && (GATTC_INCLUDED == 1)
...
void bta_dm_disable (tBTA_DM_MSG *p_data)
{
    ...
    bta_dm_cb.disabling = 1;

#if BLE_INCLUDED == 1 && BTA_GATT_INCLUDED == 1
...
#if BLE_INCLUDED == 1 && BLE_PRIVACY_SPT == 1
...
**
** Returns          1 if started to get remote name
**
*******************************************************************************/
static BOOLEAN bta_dm_read_remote_device_name (BD_ADDR bd_addr, tBT_TRANSPORT transport)
{
...
void bta_dm_set_visibility(tBTA_DM_MSG *p_data)
{
    ...
    if (p_data->set_visibility.conn_paired_only != BTA_DM_IGNORE) {
        ...
        if (p_data->set_visibility.conn_paired_only == BTA_DM_CONN_ALL) {
            bta_dm_cb.conn_paired_only = 0;
        } else {
            bta_dm_cb.conn_paired_only = 1;
        }
...
{
#if (BLE_INCLUDED == 1 && GATTC_INCLUDED == 1)
    /* need to remove all pending background connection before unpair */
    BTA_GATTC_CancelOpen(0, bd_addr, 0);
#endif
...
void bta_dm_close_acl(tBTA_DM_MSG *p_data)
{
    ...
    if (BTM_IsAclConnectionUp(p_remove_acl->bd_addr, p_remove_acl->transport)) {
        ...
        if (index != bta_dm_cb.device_list.count) {
            if (p_remove_acl->remove_dev) {
                bta_dm_cb.device_list.peer_device[index].remove_dev_pending = 1;
            }
        } else {
    ...
    /* if to remove the device from security database ? do it now */
        ...
        BTA_GATTC_CancelOpen(0, p_remove_acl->bd_addr, 0);
        /* remove all cached GATT information */
...
void bta_dm_bond (tBTA_DM_MSG *p_data)
{
    ...
    if (bta_dm_cb.p_sec_cback && (status != BTM_CMD_STARTED)) {
        ...
        /*      taken care of by memset [above]
                sec_event.auth_cmpl.key_present = 0;
                sec_event.auth_cmpl.success = 0;
        */
        sec_event.auth_cmpl.fail_reason = HCI_ERR_ILLEGAL_COMMAND;
        if (status == BTM_SUCCESS) {
            sec_event.auth_cmpl.success = 1;
        } else {
...
void bta_dm_search_cancel (tBTA_DM_MSG *p_data)
{
    ...
    if (BTM_IsInquiryActive()) {
        if (BTM_CancelInquiry() != BTM_CMD_STARTED) {
            ...
            bta_dm_search_cb.cancel_pending = 1;
        }
    }
...
{
#if BLE_INCLUDED == 1 && BTA_GATT_INCLUDED == 1  && GATTC_INCLUDED == 1
    ...
    bta_dm_search_cb.name_discover_done = 0;
...
void bta_dm_rmt_name (tBTA_DM_MSG *p_data)
{
    ...
    if ( p_data->rem_name.result.disc_res.bd_name[0] && bta_dm_search_cb.p_btm_inq_info) {
        bta_dm_search_cb.p_btm_inq_info->appl_knows_rem_name = 1;
    }
#if (SDP_INCLUDED == 1)
...
void bta_dm_disc_rmt_name (tBTA_DM_MSG *p_data)
{
    ...
    if ( p_btm_inq_info ) {
        if ( p_data->rem_name.result.disc_res.bd_name[0] ) {
            p_btm_inq_info->appl_knows_rem_name = 1;
        }
    }
#if (SDP_INCLUDED == 1)
...
#if (BLE_INCLUDED == 1 && BTA_GATT_INCLUDED == 1 && SDP_INCLUDED == 1)
    utl_freebuf((void **)&bta_dm_search_cb.p_srvc_uuid);
#endif
...
void bta_dm_search_result (tBTA_DM_MSG *p_data)
{
    ...
    if (!bta_dm_search_cb.wait_disc ) {
        ...
        bta_dm_search_cb.sdp_results = 1;
...
static void bta_dm_discover_device(BD_ADDR remote_bd_addr)
{
    ...
    if ((bta_dm_search_cb.p_btm_inq_info)
        ...
        bta_dm_search_cb.name_discover_done = 1;
    }
    ...
    if ((!bta_dm_search_cb.name_discover_done)
        ...
        if (bta_dm_read_remote_device_name(bta_dm_search_cb.peer_bdaddr, transport) == 1) {
            return;
        }
    ...
    if ( bta_dm_search_cb.services ) {
        ...
        if (bta_dm_search_cb.services_to_search) {
            ...
            if (BTM_IsAclConnectionUp(bta_dm_search_cb.peer_bdaddr, BT_TRANSPORT_BR_EDR)) {
                ...
                bta_dm_search_cb.wait_disc = 1;
            }

#if (BLE_INCLUDED == 1 && (defined BTA_GATT_INCLUDED) && (BTA_GATT_INCLUDED == 1) && SDP_INCLUDED == 1) && (GATTC_INCLUDED == 1)
            if ( bta_dm_search_cb.p_btm_inq_info ) {
...
#endif
            {
                bta_dm_search_cb.sdp_results = 0;
                bta_dm_find_services(bta_dm_search_cb.peer_bdaddr);
                return;
            }
        }
...
static void bta_dm_inq_results_cb (tBTM_INQ_RESULTS *p_inq, UINT8 *p_eir)
{
    ...
    result.inq_res.is_limited = (service_class & BTM_COD_SERVICE_LMTD_DISCOVER) ? 1 : 0;
    ...
    if ((p_inq_info = BTM_InqDbRead(p_inq->remote_bd_addr)) != NULL) {
        /* initialize remt_name_not_required to 0 so that we get the name by default */
        result.inq_res.remt_name_not_required = 0;
    ...
    if (p_inq_info) {
        ...
        if (result.inq_res.remt_name_not_required) {
            p_inq_info->appl_knows_rem_name = 1;
        }
...
static void bta_dm_inq_cmpl_cb (void *p_result)
{
    ...
    if (bta_dm_search_cb.cancel_pending == 0) {
        ...
        bta_dm_search_cb.cancel_pending = 0;
        bta_dm_search_cancel_notify(NULL);
...
#if BLE_INCLUDED == 1 && GATTS_INCLUDED == 1
...
#if ( defined(BTA_JV_INCLUDED) && BTA_JV_INCLUDED == 1 )
                                        ...
                                        BOOLEAN sc_support)
{
    ...
    if (key_type != HCI_LKEY_TYPE_AMP_WIFI && key_type != HCI_LKEY_TYPE_AMP_UWB) {
        ...
        p_auth_cmpl->key_present = 1;
        p_auth_cmpl->key_type = key_type;
        p_auth_cmpl->success = 1;
...
**
** Returns          BOOLEAN - 1, if switch is in progress
**
*******************************************************************************/
static BOOLEAN bta_dm_check_av(UINT16 event)
{
    BOOLEAN avoid_roleswitch = 0;
    BOOLEAN switching = 0;
    UINT8 i;
    tBTA_DM_PEER_DEVICE *p_dev;

#if defined(BTA_DM_AVOID_A2DP_ROLESWITCH_ON_INQUIRY) && (BTA_DM_AVOID_A2DP_ROLESWITCH_ON_INQUIRY == 1)
...
void bta_dm_acl_change(tBTA_DM_MSG *p_data)
{
    ...
    BOOLEAN         need_policy_change = 0;
    BOOLEAN         issue_unpair_cb = 0;
    ...
    if (is_new) {
        ...
        if (conn.link_down.is_removed) {
            ...
            BTA_GATTC_CancelOpen(0, p_bda, 0);
            /* remove all cached GATT information */
            BTA_GATTC_Refresh(p_bda, 0);
#endif
        }
...
#if (BLE_INCLUDED == 1 && GATTC_INCLUDED == 1)
        /* need to remove all pending background connection */
        BTA_GATTC_CancelOpen(0, remote_bd_addr, 0);
        /* remove all cached GATT information */
        BTA_GATTC_Refresh(remote_bd_addr, 0);
#endif
    }
}
#endif  ///SMP_INCLUDED == 1
...
#else /* (BTA_EIR_CANNED_UUID_LIST == 1) */
...
#endif /* (BTA_EIR_CANNED_UUID_LIST == 1) */

#if ( BTA_EIR_CANNED_UUID_LIST != 1 )&&(BTA_EIR_SERVER_NUM_CUSTOM_UUID > 0)
...
#endif /* ( BTA_EIR_CANNED_UUID_LIST != 1 )&&(BTA_EIR_SERVER_NUM_CUSTOM_UUID > 0) */
    }
...
void bta_dm_ble_observe (tBTA_DM_MSG *p_data)
{
    ...
    if (p_data->ble_observe.start) {
        ...
        if ((status = BTM_BleObserve(1, p_data->ble_observe.duration,
        ...
        status = BTM_BleObserve(0, 0, NULL, NULL);
...
void bta_dm_ble_scan (tBTA_DM_MSG *p_data)
{
    ...
    if (p_data->ble_scan.start) {
        ...
        if ((status = BTM_BleScan(1, p_data->ble_scan.duration,
        ...
        bta_dm_search_cb.p_scan_cback = NULL;
        status = BTM_BleScan(0, 0, NULL, NULL, NULL);
...
#endif // #if (BLE_50_FEATURE_SUPPORT == 1)
/*******************************************************************************
**
...
#if BLE_ANDROID_CONTROLLER_SCAN_FILTER == 1
/*******************************************************************************
**
...
static void bta_dm_gatt_disc_complete(UINT16 conn_id, tBTA_GATT_STATUS status)
{
    ...
    if (status == BTA_GATT_OK && bta_dm_search_cb.uuid_to_search > 0) {
        ...
        bta_dm_search_cb.gatt_disc_active = 0;
    }
}
#endif /* #if (GATTC_INCLUDED == 1) */
...
{
    bta_dm_search_cb.gatt_disc_active = 1;
    ...
    if (bdcmp(bta_dm_search_cb.pending_close_bda, bd_addr) == 0 &&
        ...
        BTA_GATTC_Open(bta_dm_search_cb.client_if, bd_addr, BLE_ADDR_UNKNOWN_TYPE, 1, BTA_GATT_TRANSPORT_LE, 0);
    }
}
#endif /* #if (GATTC_INCLUDED == 1) */
/*******************************************************************************
...
{
    if (bta_dm_search_cb.conn_id == BTA_GATT_INVALID_CONN_ID) {
        BTA_GATTC_CancelOpen(bta_dm_search_cb.client_if, bd_addr, 1);
    }
