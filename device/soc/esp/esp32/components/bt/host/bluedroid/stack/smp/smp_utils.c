...
void smp_proc_pairing_cmpl(tSMP_CB *p_cb)
{
    ...
    evt_data.cmplt.is_pair_cancel  = 0;

    if (p_cb->is_pair_cancel) {
        evt_data.cmplt.is_pair_cancel = 1;
    }
    ...
    if (p_cb->role == HCI_ROLE_SLAVE) {
        if(p_rec && p_rec->ble.skip_update_conn_param) {
            ...
            L2CA_EnableUpdateBleConnParams(p_cb->pairing_bda, 1);
            #endif
        }
    }
...
**                  It returns 1 if the command has invalid parameters.
**
** Returns          1 if the command has invalid parameters, 0 otherwise.
**
*******************************************************************************/
...
**                  according to specs.
**
** Returns          1 if the command size is as expected, 0 otherwise.
**
...
**                  Returns 0 if at least one of these parameters is out of range.
**
*******************************************************************************/
...
**                  Returns 0 if this parameter is out of range.
**
*******************************************************************************/
...
**                  mode, i.e. if p_cb->secure_connections_only_mode_required = 1 then we come
...
tSMP_ASSO_MODEL smp_select_association_model(tSMP_CB *p_cb)
{
    ...
    p_cb->le_secure_connections_mode_is_used = 0;
    ...
    if ((p_cb->peer_auth_req & SMP_SC_SUPPORT_BIT) && (p_cb->loc_auth_req & SMP_SC_SUPPORT_BIT)) {
        p_cb->le_secure_connections_mode_is_used = 1;
    }
...
**
** Returns          1  if the values are the same
**                  0 otherwise
**
*******************************************************************************/
...
void smp_save_secure_connections_long_term_key(tSMP_CB *p_cb)
{
    ...
    btm_sec_save_le_key(p_cb->pairing_bda, BTM_LE_KEY_LENC, (tBTM_LE_KEY_VALUE *)&lle_key, 1);
    ...
    btm_sec_save_le_key(p_cb->pairing_bda, BTM_LE_KEY_PENC, (tBTM_LE_KEY_VALUE *)&ple_key, 1);
}
...
**                  the link.
**
** Returns          0 if out of resources, 1 otherwise.
**
*******************************************************************************/
...
**
** Returns          1 - OOB data has to be provided by application
**                  0 - otherwise (unexpected)
**
*******************************************************************************/
