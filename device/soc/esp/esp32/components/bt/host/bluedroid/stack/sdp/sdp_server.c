        ...
        UINT8 *p_req_end)
{
    ...
    tSDP_ATTRIBUTE *p_attr;
    BT_HDR         *p_buf;
    BOOLEAN         maxxed_out = 0, is_cont = 0;
    UINT8           *p_seq_start;
    ...
    if (*p_req) {
        ...
        p_ccb->cont_info.prev_sdp_rec = NULL;
        p_ccb->cont_info.next_attr_index = 0;
        p_ccb->cont_info.last_attr_seq_desc_sent = 0;
        p_ccb->cont_info.attr_offset = 0;
    }
    ...
    for (p_rec = sdp_db_service_search (p_ccb->cont_info.prev_sdp_rec, &uid_seq); p_rec; p_rec = sdp_db_service_search (p_rec, &uid_seq)) {
        ...
        p_seq_start = p_rsp;
        if (p_ccb->cont_info.last_attr_seq_desc_sent == 0) {
        ...
        if (p_ccb->cont_info.last_attr_seq_desc_sent == 0) {
            ...
            if (seq_len != 0) {
                ...
                if (maxxed_out) {
                    p_ccb->cont_info.last_attr_seq_desc_sent = 1;
                }
            } else {
                p_rsp = p_seq_start;
            }
