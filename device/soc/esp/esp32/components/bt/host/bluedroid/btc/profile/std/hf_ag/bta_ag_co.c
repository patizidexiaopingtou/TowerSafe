...
static void bta_hf_dec_init(void)
{
    ...
    OI_STATUS status = OI_CODEC_SBC_DecoderReset(&bta_ag_co_cb.decoder_context, bta_ag_co_cb.decoder_context_data,
                                       HF_SBC_DEC_CONTEXT_DATA_LEN * sizeof(OI_UINT32), 1, 1, 0, 1);
...
// PLC_INCLUDED will be set to 1 when enabling Wide Band Speech
#if (PLC_INCLUDED == 1)
    switch(status) {
        case OI_OK:
        {
            bta_hf_ct_plc.first_good_frame_found = 1;
...
void bta_ag_sco_co_close(void)
{
    ...
    if (hf_air_mode == BTM_SCO_AIR_MODE_TRANSPNT) {
        ...
        bta_hf_ct_plc.first_good_frame_found = 0;
#if (HFP_DYNAMIC_MEMORY == 1)
        osi_free(bta_hf_ct_plc_ptr);
        bta_hf_ct_plc_ptr = NULL;
#endif  /// (HFP_DYNAMIC_MEMORY == 1)
