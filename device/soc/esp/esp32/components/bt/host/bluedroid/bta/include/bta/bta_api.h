...
typedef struct {
    ...
    BOOLEAN         min_16_digit;       /* 1 if the pin returned must be at least 16 digits */
} tBTA_DM_PIN_REQ;
...
typedef struct {
    ...
    BOOLEAN         success;            /* 1 of authentication succeeded, 0 if failed. */
    UINT8           fail_reason;        /* The HCI reason/error code for when success=0 */
...
typedef struct {
    ...
    BOOLEAN         is_removed;         /* 1 if device is removed when link is down */
#if BLE_INCLUDED == 1
    tBTA_TRANSPORT  link_type;
#endif
} tBTA_DM_LINK_DOWN;
...
typedef struct {
    tBTA_STATUS     result;    /* 1 of bond cancel succeeded, 0 if failed. */
...
typedef struct {
    ...
    BOOLEAN         is_limited;             /* 1, if the limited inquiry bit is set in the CoD */
...
**
** Parameters:      add              - add static passkey when add is 1
