    ...
    case GATT_READ_BY_HANDLE:
        if (!p_clcb->counter) {
            ...
            if (!p_clcb->first_read_blob_after_read) {
                p_clcb->first_read_blob_after_read = 1;
            } else {
                p_clcb->first_read_blob_after_read = 0;
            }
...
**
** Returns          1: write long is terminated; 0 keep sending.
**
*******************************************************************************/
    ...
    while (len >= (handle_len + value_len)) {
        ...
        /* discover included service */
            ...
            if (value_len == 6) {
                ...
                p_clcb->read_uuid128.wait_for_read_rsp = 1;
                           ...
                           UINT16 len, UINT8 *p_data)
{
    ...
    if (p_clcb->operation == GATTC_OPTYPE_READ) {
        ...
        if (p_clcb->operation == GATTC_OPTYPE_DISCOVERY &&
            ...
            p_clcb->read_uuid128.wait_for_read_rsp = 0;
            if (len == LEN_UUID_128) {
...
#endif  /* BLE_INCLUDED == 1 && GATTC_INCLUDED == 1 */
