...
#if (defined BTC_SPP_INCLUDED && BTC_SPP_INCLUDED == 1)
    ...
    while (1) {
        ...
        if ((slot = spp_local_param.spp_slots[serial]) != NULL) {
            ...
            if (!enqueue_status) {
                ...
                tx_event_group_val =
                    xEventGroupWaitBits(spp_local_param.tx_event_group, SLOT_WRITE_BIT(serial) | SLOT_CLOSE_BIT(serial), pd1,
                                        pd0, VFS_WRITE_TIMEOUT / portTICK_PERIOD_MS);
...
#endif ///defined BTC_SPP_INCLUDED && BTC_SPP_INCLUDED == 1
