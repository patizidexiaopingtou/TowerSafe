...
#if( defined GATTS_INCLUDED ) && (GATTS_INCLUDED == 1)
...
void btif_gatts_add_bonded_dev_from_nv(BD_ADDR bda)
{
    ...
    if (!found) {
        if (p_cb->num_clients < BTIF_GATTS_MAX_SRV_CHG_CLT_SIZE) {
            ...
            p_cb->srv_chg[p_cb->num_clients].srv_changed = 0;
            p_cb->num_clients++;
        }
    }
}
...
**
** Parameter        is_add: 1 is to add a handle range; otherwise is to delete.
...
**
** Returns          1 - if the request is processed successfully and
**                         the response is returned in p_rsp.
**                  0 - if the request can not be processed
**
*******************************************************************************/
