...
static void bta_ag_sco_disc_cback(UINT16 sco_idx)
{
    ...
    {
#if (BTM_SCO_HCI_INCLUDED == 1 )
        tBTM_STATUS status = BTM_ConfigScoPath(BTM_SCO_ROUTE_PCM, NULL, NULL, 1);
        ...
        if(bta_ag_cb.sco.p_curr_scb->inuse_codec == BTA_AG_CODEC_MSBC)
        {
            ...
            if (bta_ag_sco_is_opening (bta_ag_cb.sco.p_curr_scb))
            {
                ...
                else
                {
                    ...
                    bta_ag_cb.sco.p_curr_scb->codec_fallback = 1;
                }
            }
        }
...
**                  If only_active is 1, then SCO is only removed if connected
**
** Returns          BOOLEAN   - 1 if Sco removal was started
**
*******************************************************************************/
static BOOLEAN bta_ag_remove_sco(tBTA_AG_SCB *p_scb, BOOLEAN only_active)
{
    BOOLEAN     removed_started = 0;
    tBTM_STATUS	status;
    ...
    if (event == BTM_ESCO_CONN_REQ_EVT)
    {
            ...
            ((p_scb = bta_ag_scb_by_idx(handle)) != NULL) && p_scb->svc_conn)
        {
            ...
            else    /* Begin a transfer: Close current SCO before responding */
            {
                ...
                if (!bta_ag_remove_sco(bta_ag_cb.sco.p_curr_scb, 1))
                {
...
**
** Description      Create a sco connection and is is_orig is 1 means AG originate
**                  this connection, if 0 it's peer device originate the connection.
**
**
** Returns          void
**
...
static void bta_ag_create_sco(tBTA_AG_SCB *p_scb, BOOLEAN is_orig)
{
    ...
    if (is_orig)
    {
        ...
        if (p_scb->conn_service == BTA_AG_HFP && p_scb->peer_version >= HFP_VERSION_1_5 && !p_scb->retry_with_sco_only)
        {
               ...
               ||!((params.packet_types & ~(BTM_ESCO_LINK_ONLY_MASK | BTM_SCO_LINK_ONLY_MASK)) ^ BTA_AG_NO_EDR_ESCO))
            {
                ...
                {
                    p_scb->retry_with_sco_only = 1;
                    APPL_TRACE_API("Setting retry_with_sco_only to 1");
                }
                else    /* Do not use SCO when using mSBC */
                {
                    p_scb->retry_with_sco_only = 0;
                    APPL_TRACE_API("Setting retry_with_sco_only to 0");
                }
#else
                p_scb->retry_with_sco_only = 1;
                APPL_TRACE_API("Setting retry_with_sco_only to 1");
    ...
    if (esco_codec == BTA_AG_CODEC_MSBC)
        ...
        BTM_ConfigScoPath(sco_route, bta_ag_sco_read_cback, NULL, 1);
#endif
        bta_ag_cb.sco.cur_idx = p_scb->sco_idx;
    }
    else{
...
**
**
** Returns          1 if T1 settings has to be used, 0 otherwise
**
*******************************************************************************/
        ...
        case BTA_AG_SCO_OPEN_XFER_ST:
            switch (event)
            {
                ...
                case BTA_AG_SCO_CONN_CLOSE_E:
                    ...
                    bta_ag_create_sco(p_scb, 0);    /* Back into listen mode */
        ...
        case BTA_AG_SCO_OPEN_ST:
            switch (event)
            {
                ...
                case BTA_AG_SCO_XFER_E:
                    ...
                    bta_ag_remove_sco(p_sco->p_curr_scb, 1);
                    /* save xfer scb */
                    p_sco->p_xfer_scb = p_scb;
                    p_sco->state = BTA_AG_SCO_CLOSE_XFER_ST;
                    break;
        ...
        case BTA_AG_SCO_CLOSE_XFER_ST:
            switch (event)
            {
                ...
                case BTA_AG_SCO_CONN_CLOSE_E:
                    ...
                    bta_ag_create_sco(p_scb, 0);
                    bta_ag_remove_sco(p_sco->p_xfer_scb, 0);
...
#else
                    /* create sco connection to peer */
                    bta_ag_create_sco(p_sco->p_xfer_scb, 1);
                    p_sco->p_xfer_scb = NULL;
                    p_sco->state = BTA_AG_SCO_OPENING_ST;
#endif
                    break;
...
**
**
** Returns          1 if sco open for this scb, 0 otherwise.
**
*******************************************************************************/
...
**
**
** Returns          1 if sco is in Opening state for this scb, 0 otherwise.
**
*******************************************************************************/
...
{
    if(set_reset == 0)    /* reset the parameters to default */
    {
        bta_ag_cb.sco.param_updated = 0;
        APPL_TRACE_DEBUG("bta_ag_set_esco_param : Resetting ESCO parameters to default");
    }
    else
    {
        bta_ag_cb.sco.param_updated = 1;
