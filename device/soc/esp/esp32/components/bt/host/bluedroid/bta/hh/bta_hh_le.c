...
BOOLEAN bta_hh_le_add_hid_srvc_entry(tBTA_HH_DEV_CB *p_dev_cb, UINT8 idx)
{
    ...
    if (idx < BTA_HH_LE_HID_SRVC_MAX) {
        p_dev_cb->hid_srvc[idx].in_use = 0;
        added = 0;
    } else {
...
void bta_hh_le_open_conn(tBTA_HH_DEV_CB *p_cb, BD_ADDR remote_bda)
{
    ...
    BTA_GATTC_Open(bta_hh_cb.gatt_if, remote_bda, BLE_ADDR_UNKNOWN_TYPE, 0, BTA_GATT_TRANSPORT_LE, 0);
}
        ...
        UINT16 char_uuid, UINT8 char_inst_id, UINT16 char_descp_uuid)
{
    ...
    bta_hh_le_fill_16bits_srvc_id(0, srvc_inst_id, srvc_uuid, &char_id.srvc_id);
                                    ...
                                    UINT8 proto_mode, BOOLEAN register_ba)
{
    ...
    for (i = 0; i < BTA_HH_LE_RPT_MAX; i ++, p_rpt ++) {
        if (p_rpt->rpt_type == BTA_HH_RPTT_INPUT) {
            ...
            bta_hh_le_fill_16bits_srvc_id(0, BTA_HH_LE_RPT_GET_SRVC_INST_ID(p_rpt->inst_id), srvc_uuid, &char_id.srvc_id);
...
#endif
        bta_hh_le_register_input_notif(p_cb, 0, p_cb->mode, 0);
        bta_hh_sm_execute(p_cb, BTA_HH_OPEN_CMPL_EVT, NULL);

#if (BTA_HH_LE_RECONN == 0)
        if (p_cb->status == BTA_HH_OK) {
            bta_hh_le_add_dev_bg_conn(p_cb, 0);
        }
#endif
    }
}
                                     ...
                                     UINT16 clt_cfg_value)
{
    ...
    bta_hh_le_fill_16bits_srvc_id(0, srvc_inst_id, srvc_uuid16, &char_id.srvc_id);
...
BOOLEAN bta_hh_le_set_protocol_mode(tBTA_HH_DEV_CB *p_cb, tBTA_HH_PROTO_MODE mode)
{
    ...
    if ((p_cb->hid_srvc[BTA_HH_LE_SRVC_DEF].option_char & BTA_HH_LE_PROTO_MODE_BIT) == 0) {
        ...
        bta_hh_le_fill_16bits_srvc_id(0, 0, UUID_SERVCLASS_LE_HID, &char_id.srvc_id);
...
void bta_hh_security_cmpl(tBTA_HH_DEV_CB *p_cb, tBTA_HH_DATA *p_buf)
{
    ...
    if (p_cb->status == BTA_HH_OK) {
        ...
        if (p_cb->app_id != 0 && p_cb->hid_srvc[BTA_HH_LE_SRVC_DEF].in_use) {
            ...
            if (bta_hh_le_set_protocol_mode(p_cb, p_cb->mode) == 0) {
...
{
    if (p_cb == NULL || p_cb->security_pending == 0 ||
            p_buf == NULL || p_buf->le_enc_cmpl.client_if != bta_hh_cb.gatt_if) {
        return;
    }

    p_cb->security_pending = 0;
...
static void bta_hh_le_search_hid_included(tBTA_HH_DEV_CB *p_dev_cb)
{
    ...
    bta_hh_le_fill_16bits_srvc_id(0, p_dev_cb->cur_srvc_index, UUID_SERVCLASS_LE_HID, &srvc_id);
    ...
    p_dev_cb->hid_srvc[p_dev_cb->cur_srvc_index].expl_incl_srvc = 0;
...
void bta_hh_le_write_cmpl(tBTA_HH_DEV_CB *p_dev_cb, tBTA_HH_DATA *p_buf)
{
    ...
    case GATT_UUID_HID_PROTO_MODE:
        ...
        if (p_data->status == BTA_GATT_OK) {
            bta_hh_le_register_input_notif(p_dev_cb, p_data->srvc_id.id.inst_id, p_dev_cb->mode, 0);
            cback_data.status = BTA_HH_OK;
        } else {
            cback_data.status =  BTA_HH_ERR;
        }
...
void bta_hh_le_get_rpt(tBTA_HH_DEV_CB *p_cb, UINT8 srvc_inst, tBTA_HH_RPT_TYPE r_type, UINT8 rpt_id)
{
    ...
    bta_hh_le_fill_16bits_srvc_id(0, srvc_inst, srvc_uuid, &char_id.srvc_id);
                         ...
                         BT_HDR *p_buf, UINT16 w4_evt )
{
    ...
    bta_hh_le_fill_16bits_srvc_id(0, srvc_inst, UUID_SERVCLASS_LE_HID, &char_id.srvc_id);
    bta_hh_le_fill_16bits_char_id(p_rpt->inst_id, p_rpt->uuid, &char_id.char_id);
...
void bta_hh_le_suspend(tBTA_HH_DEV_CB *p_cb, tBTA_HH_TRANS_CTRL_TYPE ctrl_type)
{
    ...
    for (i = 0; i < BTA_HH_LE_HID_SRVC_MAX; i ++) {
        bta_hh_le_fill_16bits_srvc_id(0, i, UUID_SERVCLASS_LE_HID, &char_id.srvc_id);
...
static void bta_hh_le_add_dev_bg_conn(tBTA_HH_DEV_CB *p_cb, BOOLEAN check_bond)
{
    ...
    if (/*p_cb->dscp_info.flag & BTA_HH_LE_NORMAL_CONN &&*/
        ...
        BTA_GATTC_Open(bta_hh_cb.gatt_if, p_cb->addr, BLE_ADDR_UNKNOWN_TYPE, 0, BTA_GATT_TRANSPORT_LE, 0);
        p_cb->in_bg_conn = 0;
...
void bta_hh_le_remove_dev_bg_conn(tBTA_HH_DEV_CB *p_dev_cb)
{
    if (p_dev_cb->in_bg_conn) {
        p_dev_cb->in_bg_conn = 0;

        BTA_GATTC_CancelOpen(bta_hh_cb.gatt_if, p_dev_cb->addr, 0);
    }
}
...
void bta_hh_le_update_scpp(tBTA_HH_DEV_CB *p_dev_cb, tBTA_HH_DATA *p_buf)
{
    ...
    bta_hh_le_fill_16bits_srvc_id(0, BTA_HH_SCPP_INST_DEF, UUID_SERVCLASS_SCAN_PARAM, &char_id.srvc_id);
...
static void bta_hh_le_search_scps_chars(tBTA_HH_DEV_CB *p_cb)
{
    ...
    p_cb->scps_supported = 0;
    bta_hh_le_fill_16bits_srvc_id(0, 0, UUID_SERVCLASS_SCAN_PARAM, &srvc_id);
                                      ...
                                      UINT8 num_rpt)
{
    ...
    if (num_rpt != 0) { /* no cache is found */
        p_cb->hid_srvc[BTA_HH_LE_RPT_GET_SRVC_INST_ID(p_rpt_cache->inst_id)].in_use = 0;
