...
tSMP_STATUS SMP_Pair (BD_ADDR bd_addr)
{
    ...
    if (p_cb->state != SMP_STATE_IDLE || p_cb->flags & SMP_PAIR_FLAGS_WE_STARTED_DD ||
        ...
        if (!L2CA_ConnectFixedChnl (L2CAP_SMP_CID, bd_addr, BLE_ADDR_UNKNOWN_TYPE, 0)) {
...
tSMP_STATUS SMP_BR_PairWith (BD_ADDR bd_addr)
{
    ...
    if (!L2CA_ConnectFixedChnl (L2CAP_SMP_BR_CID, bd_addr, BLE_ADDR_UNKNOWN_TYPE, 0)) {
...
** Parameters       bd_addr - peer device bd address.
**
** Returns          1 - Pairining is cancelled
**
*******************************************************************************/
...
**
**  Returns         Boolean - 1: creation of local SC OOB data set started.
*******************************************************************************/
