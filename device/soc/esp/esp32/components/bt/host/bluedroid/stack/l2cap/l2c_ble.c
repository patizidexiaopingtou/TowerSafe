...
**
**  Return value:   1 if connection was cancelled
**
*******************************************************************************/
...
**  Parameters:     BD Address of remote
**
**  Return value:   1 if update started
**
*******************************************************************************/
                                  ...
                                  UINT16 latency, UINT16 timeout)
{
    ...
    if(l2cble_start_conn_update(p_lcb) == 1) {
                               ...
                               UINT16 conn_interval, UINT16 conn_latency, UINT16 conn_timeout)
{
    ...
    if (!p_lcb) {
        p_lcb = l2cu_allocate_lcb (bda, 0, BT_TRANSPORT_LE);
        if (!p_lcb) {
#if (SMP_INCLUDED == 1)
...
void l2cble_conn_comp(UINT16 handle, UINT8 role, BD_ADDR bda, tBLE_ADDR_TYPE type,
                      UINT16 conn_interval, UINT16 conn_latency, UINT16 conn_timeout)
{
    btm_ble_update_link_topology_mask(role, 1);
...
**
**  Return value:   true if successfully sending the request to peer device, else 0.
**
*******************************************************************************/
static BOOLEAN l2cble_start_conn_update (tL2C_LCB *p_lcb)
{
    UINT16 min_conn_int, max_conn_int, slave_latency, supervision_tout;
#if (defined BLE_LLT_INCLUDED) && (BLE_LLT_INCLUDED == 1) && (BLE_SLAVE_UPD_CONN_PARAMS == 1)
    tACL_CONN *p_acl_cb = btm_bda_to_acl(p_lcb->remote_bd_addr, BT_TRANSPORT_LE);
#endif    /* defined BLE_LLT_INCLUDED) && (BLE_LLT_INCLUDED == 1) && (BLE_SLAVE_UPD_CONN_PARAMS == 1 */
...
**
** Returns          1 connection initiated, 0 otherwise.
**
*******************************************************************************/
...
#endif // ( (defined BLE_PRIVACY_SPT) && (BLE_PRIVACY_SPT == 1) && (!CONTROLLER_RPA_LIST_ENABLE))
...
**
** Returns          1 if successful, 0 if connection not started.
**
*******************************************************************************/
...
void l2cble_process_data_length_change_event(UINT16 handle, UINT16 tx_data_len, UINT16 rx_data_len)
{
    ...
    if(p_acl) {
        p_acl->data_len_updating = 0;
        if(p_acl->data_len_waiting) {
            p_acl->data_len_waiting = 0;
...
**
** Returns          1 - security procedures are started
**                  0 - failure
**
*******************************************************************************/
