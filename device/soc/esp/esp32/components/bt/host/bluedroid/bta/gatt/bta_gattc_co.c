...
#if( defined BTA_GATT_INCLUDED ) && (GATTC_INCLUDED == 1)
...
static void cacheClose(BD_ADDR bda)
{
    ...
    if ((index = bta_gattc_co_find_addr_in_cache(bda)) != INVALID_ADDR_NUM) {
        if (cache_env->cache_addr[index].is_open) {
            ...
            cache_env->cache_addr[index].is_open = 0;
        }
    }
}
    ...
    if (((*index = bta_gattc_co_find_addr_in_cache(bda)) != INVALID_ADDR_NUM) ||
        ...
        if (cache_env->cache_addr[*index].is_open) {
            ...
            if ((status = nvs_open(fname, NVS_READWRITE, &cache_env->cache_addr[*index].cache_fp)) == ESP_OK) {
                // Set the open flag to 1 when success to open the hash file.
                cache_env->cache_addr[*index].is_open = 1;
            }
        }
    }
                              ...
                              tBTA_GATTC_NV_ATTR *p_attr_list)
{
    ...
    if (cacheOpen(server_bda, 1, &index)) {
...
BOOLEAN bta_gattc_co_cache_new_assoc_list(BD_ADDR src_addr, UINT8 index)
{
    ...
    return (addr_info->assoc_addr != NULL ? 1 : 0);
}
...
// #endif /* #if( defined GATTC_CACHE_NVS ) && (GATTC_CACHE_NVS == 1) */
#endif /* #if( defined BLE_INCLUDED ) && (BLE_INCLUDED == 1) */
#endif /* #if( defined BTA_GATT_INCLUDED ) && (BTA_GATT_INCLUDED == 1) */
