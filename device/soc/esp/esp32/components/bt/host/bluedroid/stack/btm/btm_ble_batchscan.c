                                    ...
                                    tBTM_BLE_REF_VALUE ref_value)
{
    ...
    if (BTM_BLE_SCAN_INVALID_STATE == ble_batchscan_cb.cur_state ||
        ...
        status = btm_ble_enable_disable_batchscan(1);
        if (BTM_CMD_STARTED != status) {
            return status;
        }
...
tBTM_STATUS BTM_BleDisableBatchScan(tBTM_BLE_REF_VALUE ref_value)
{
    ...
    status = btm_ble_enable_disable_batchscan(0);
    if (BTM_CMD_STARTED == status) {
...
void btm_ble_batchscan_init(void)
{
    ...
    BTM_RegisterForVSEvents(btm_ble_batchscan_filter_track_adv_vse_cback, 1);
}
