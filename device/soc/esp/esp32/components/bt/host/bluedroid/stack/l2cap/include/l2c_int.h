...
typedef struct {
    ...
    BOOLEAN     remote_busy;                /* 1 if peer has flowed us off           */
    BOOLEAN     local_busy;                 /* 1 if we have flowed off the peer      */
...
typedef struct t_l2c_ccb {
    BOOLEAN             in_use;                 /* 1 when in use, 0 when not */
...
#define CCB_FLAG_SENT_PENDING   0x02            /* already sent pending response */
    ...
    BOOLEAN             peer_cfg_already_rejected; /* If mode rejected once, set to 1 */
    BOOLEAN             out_cfg_fcr_present;    /* 1 if cfg response shoulkd include fcr options */
...
#define L2CAP_BYPASS_FCS    (L2CAP_CFG_FCS_OUR | L2CAP_CFG_FCS_PEER)
    ...
    BOOLEAN             is_flushable;                   /* 1 if channel is flushable     */
#endif

#if (L2CAP_NUM_FIXED_CHNLS > 0) || (L2CAP_UCD_INCLUDED == 1)
...
typedef struct t_l2c_linkcb {
    ...
    BOOLEAN             partial_segment_being_sent; /* Set 1 when a partial segment  */
    /* is being sent.                   */
    BOOLEAN             w4_info_rsp;                /* 1 when info request is active */
...
typedef struct {
    ...
    BOOLEAN         disallow_switch;                /* 0, to allow switch at create conn */
    ...
    BOOLEAN         is_flush_active;                /* 1 if an HCI_Enhanced_Flush has been sent */
#endif
...
#if (defined(L2CAP_HIGH_PRI_CHAN_QUOTA_IS_CONFIGURABLE) && (L2CAP_HIGH_PRI_CHAN_QUOTA_IS_CONFIGURABLE == 1))
    UINT16              high_pri_min_xmit_quota;    /* Minimum number of ACL credit for high priority link */
#endif /* (L2CAP_HIGH_PRI_CHAN_QUOTA_IS_CONFIGURABLE == 1) */
...
*/
#if (!defined(L2CAP_HIGH_PRI_CHAN_QUOTA_IS_CONFIGURABLE) || (L2CAP_HIGH_PRI_CHAN_QUOTA_IS_CONFIGURABLE == 0))
...
************************************
*/
#if (!defined L2C_DYNAMIC_MEMORY) || (L2C_DYNAMIC_MEMORY == 0)
extern tL2C_CB  l2cb;
#else
...
#define L2C_LINK_CHECK_POWER_MODE(x) (0)
...
#if (defined BLE_LLT_INCLUDED) && (BLE_LLT_INCLUDED == 1)
