...
void rfc_mx_sm_state_idle (tRFC_MCB *p_mcb, UINT16 event, void *p_data)
{
    ...
    case RFC_EVENT_DISC:
        rfc_send_dm (p_mcb, RFCOMM_MX_DLCI, 1);
        return;

    case RFC_EVENT_UIH:
        rfc_send_dm (p_mcb, RFCOMM_MX_DLCI, 0);
        return;
    }
...
void rfc_mx_sm_state_disc_wait_ua (tRFC_MCB *p_mcb, UINT16 event, void *p_data)
{
    ...
    case RFC_EVENT_TIMEOUT:
        ...
        if (p_mcb->restart_required) {
            ...
            p_mcb->is_initiator     = 1;
            p_mcb->restart_required = 0;
            p_mcb->local_cfg_sent   = 0;
            p_mcb->peer_cfg_rcvd    = 0;
...
static void rfc_mx_send_config_req (tRFC_MCB *p_mcb)
{
    ...
    /* Defaults set by memset
        cfg.flush_to_present = 0;
        cfg.qos_present      = 0;
        cfg.fcr_present      = 0;
        cfg.fcr.mode         = L2CAP_FCR_BASIC_MODE;
        cfg.fcs_present      = 0;
        cfg.fcs              = N/A when fcs_present is 0;
    */
