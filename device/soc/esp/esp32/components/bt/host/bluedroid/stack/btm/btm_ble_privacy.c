...
**                  add_entry: 1 for add entry, 0 for remove entry
**
** Returns          void
**
*******************************************************************************/
...
BOOLEAN btm_ble_exe_disable_resolving_list(void)
{
    ...
    if (!controller_get_interface()->supports_ble_privacy()) {
        btm_ble_vendor_enable_irk_feature(0);
    } else {
...
void btm_ble_exe_enable_resolving_list(void)
{
    ...
    if (!controller_get_interface()->supports_ble_privacy()) {
        btm_ble_vendor_enable_irk_feature(1);
    } else {
...
**
** Returns          1 if device added, otherwise falase.
**
*******************************************************************************/
    ...
    if (p_dev_rec != NULL && /* RPA is being used and PID is known */
        ...
        if (!(p_dev_rec->ble.in_controller_list & BTM_RESOLVING_LIST_BIT) &&
                btm_ble_brcm_find_resolving_pending_entry(p_dev_rec->bd_addr,
                        BTM_BLE_META_ADD_IRK_ENTRY) == 0) {
            if (btm_cb.ble_ctr_cb.resolving_list_avail_size > 0) {
                if (rl_mask) {
                    if (!btm_ble_disable_resolving_list (rl_mask, 0)) {
                        return 0;
                    }
                }

                btm_ble_update_resolving_list(p_dev_rec->bd_addr, 1);
                if (controller_get_interface()->supports_ble_privacy()) {
                    BD_ADDR dummy_bda = {0};
...
void btm_ble_resolving_list_remove_dev(tBTM_SEC_DEV_REC *p_dev_rec)
{
    ...
    if ((p_dev_rec->ble.in_controller_list & BTM_RESOLVING_LIST_BIT) &&
            btm_ble_brcm_find_resolving_pending_entry(p_dev_rec->bd_addr,
                    BTM_BLE_META_REMOVE_IRK_ENTRY) == 0) {
        btm_ble_update_resolving_list( p_dev_rec->bd_addr, 0);
...
void btm_ble_enable_resolving_list_for_platform (UINT8 rl_mask)
{
    ...
    if (btm_cb.ble_ctr_cb.wl_state == BTM_BLE_WL_IDLE) {
        if (controller_get_interface()->get_ble_resolving_list_max_size() >
            ...
            btm_ble_disable_resolving_list(rl_mask, 1);
        }
        return;
    }
