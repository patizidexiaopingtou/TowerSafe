...
#if (defined BLE_INCLUDED && BLE_INCLUDED == 1 && GATTS_INCLUDED == 1)
...
void gap_ble_dealloc_clcb(tGAP_CLCB *p_clcb)
{
    ...
    while ((p_q = (tGAP_BLE_REQ *)fixed_queue_dequeue(p_clcb->pending_req_q, 0)) != NULL) {
        ...
        if (p_q->p_cback != NULL) {
            (*p_q->p_cback)(0, p_clcb->bda, 0, NULL);
        }
...
**
** Returns           1 is successul; 0 otherwise
**
*******************************************************************************/
...
void gap_attr_db_init(void)
{
    ...
    service_handle = GATTS_CreateService (gap_cb.gatt_if, &uuid, 0, GAP_MAX_ATTR_NUM, 1);
...
#if ((defined BTM_PERIPHERAL_ENABLED) && (BTM_PERIPHERAL_ENABLED == 1))
...
**
** Returns          1 if read started, else 0 if GAP is busy
**
*******************************************************************************/
                                     ...
                                     tGATT_TRANSPORT transport)
{
    ...
    if (p_clcb != NULL) {
        if (connected) {
            ...
            p_clcb->connected = 0;
            gap_ble_cl_op_cmpl(p_clcb, 0, 0, NULL);
            /* clean up clcb */
            gap_ble_dealloc_clcb(p_clcb);
        }
    }
...

{
    ...
    case GATT_UUID_GAP_PREF_CONN_PARAM:
        ...
        gap_ble_cl_op_cmpl(p_clcb, 1, 0, NULL);
        break;
    ...
    case GATT_UUID_GAP_DEVICE_NAME:
        ...
        gap_ble_cl_op_cmpl(p_clcb, 1, len, pp);
        break;

    case GATT_UUID_GAP_CENTRAL_ADDR_RESOL:
        gap_ble_cl_op_cmpl(p_clcb, 1, 1, pp);
        break;
    }
}
...
BOOLEAN gap_ble_accept_cl_operation(BD_ADDR peer_bda, UINT16 uuid, tGAP_BLE_CMPL_CBACK *p_cback)
{
    ...
    /* hold the link here */
    if (!GATT_Connect(gap_cb.gatt_if, p_clcb->bda, BLE_ADDR_UNKNOWN_TYPE, 1, BT_TRANSPORT_LE, 0)) {
        return started;
    }
...
BOOLEAN GAP_BleCancelReadPeerDevName (BD_ADDR peer_bda)
{
    ...
    if (!p_clcb->connected) {
        if (!GATT_CancelConnect(gap_cb.gatt_if, peer_bda, 1)) {
