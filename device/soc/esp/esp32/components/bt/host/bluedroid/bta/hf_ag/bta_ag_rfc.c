...
void bta_ag_start_servers(tBTA_AG_SCB *p_scb, tBTA_SERVICE_MASK services)
{
    ...
    for (int i = 0; i < BTA_AG_NUM_IDX && services != 0; i++, services >>= 1) {
        ...
        if (services & 1) {
            BTM_SetSecurityLevel(0, "", bta_ag_sec_id[i], p_scb->serv_sec_mask,
            ...
            bta_ag_port_status =  RFCOMM_CreateConnection(bta_ag_uuid[i], bta_ag_cb.profile[i].scn,
                1, BTA_AG_MTU, (UINT8 *) bd_addr_any, &(p_scb->serv_handle[i]),
...
**
** Description      Returns 1 if all servers are closed.
**
**
** Returns          1 if all servers are closed, 0 otherwise
**
*******************************************************************************/
...
{
    BTM_SetSecurityLevel(1, "", bta_ag_sec_id[p_scb->conn_service],
    ...
    if (RFCOMM_CreateConnection(bta_ag_uuid[p_scb->conn_service], p_scb->peer_scn,
            0, BTA_AG_MTU, p_scb->peer_addr, &(p_scb->conn_handle),
