...
BOOLEAN         in_use;
UINT8           link_role;
BOOLEAN         link_up_issued;     /* 1 if busy_level link up has been issued */
...
#define BTM_INQ_REMNAME_STATE       4   /* Remote name requests are active  */
    ...
    BOOLEAN          no_inc_ssp;        /* 1, to stop inquiry on incoming SSP */
#if (defined(BTA_HOST_INTERLEAVE_SEARCH) && BTA_HOST_INTERLEAVE_SEARCH == 1)
    btm_inq_state    next_state;        /*interleaving state to determine next mode to be inquired*/
#endif
} tBTM_INQUIRY_VAR_ST;

/* The MSB of the clock offset field indicates that the offset is valid if 1 */
...
#define BTM_SCO_XMIT_QUEUE_HIGH_WM  20
    ...
    BOOLEAN          is_orig;           /* 1 if the originator       */
    BOOLEAN          rem_bd_known;      /* 1 if remote BD addr known */
} tSCO_CONN;
...
typedef struct {
    ...
    BOOLEAN              esco_supported;    /* 1 if 1.2 cntlr AND supports eSCO links */
...
#endif
#if (BLE_PRIVACY_SPT == 1 && (!CONTROLLER_RPA_LIST_ENABLE))
...
#define BTM_SEC_STATE_DISCONNECTING_BOTH 9 /* disconnecting BR/EDR and BLE */
    ...
    BOOLEAN     is_originator;          /* 1 if device is originating connection */
#if (L2CAP_UCD_INCLUDED == 1)
    BOOLEAN     is_ucd;                 /* 1 if device is sending or receiving UCD */
    /* if incoming security failed, received UCD will be discarded */
#endif
    BOOLEAN     role_master;            /* 1 if current mode is master     */
...
#define BTM_SM4_CONN_PEND   0x40        /* set this bit to indicate accepting acl conn; to be cleared on btm_acl_created */
    UINT8       sm4;                    /* BTM_SM4_1, if the peer supports SM4 */
    ...
    BOOLEAN     remote_features_needed; /* set to 1 if the local device is in */
    ...
    BOOLEAN             new_encryption_key_is_p256; /* Set to 1 when the newly generated LK
                                                    ** is generated from P-256.
                                                    ** Link encrypted with such LK can be used
                                                    ** for SM over BR/EDR.
                                                    */
    BOOLEAN no_smp_on_br;       /* if set to 1 then SMP on BR/EDR doesn't */
...
#define BTM_SEC_IS_SM4(sm) ((BOOLEAN)(BTM_SM4_1 == ((sm)&BTM_SM4_1)))
#define BTM_SEC_IS_SM4_LEGACY(sm) ((BOOLEAN)(BTM_SM4_KNOWN == ((sm)&BTM_SM4_1)))
#define BTM_SEC_IS_SM4_UNKNOWN(sm) ((BOOLEAN)(BTM_SM4_UNKNOWN == ((sm)&BTM_SM4_1)))
...
#endif
    BOOLEAN          pin_type;                   /* 1 if PIN type is fixed */
    UINT8            pin_code_len;               /* Bonding information */
    PIN_CODE         pin_code;                   /* PIN CODE if pin type is fixed */
    BOOLEAN          connectable;                /* If 1 page scan should be enabled */
...
#define CONN_ORIENT_TERM                0
#define CONN_ORIENT_ORIG                1
typedef BOOLEAN CONNECTION_TYPE;

#endif /* (L2CAP_UCD_INCLUDED == 1) */
...
#if (CLASSIC_BT_INCLUDED == 1)
    UINT8       btm_scn[BTM_MAX_SCN];        /* current SCNs: 1 if SCN is in use */
...
#define BTM_SEC_MAX_RMT_NAME_CALLBACKS  2
    ...
    BOOLEAN                  sec_req_pending;       /*   1 if a request is pending */
#if (CLASSIC_BT_INCLUDED == 1)
...
// btla-specific --
...
#endif // SMP_INCLUDED == 1 || BT_CLASSIC_ENABLED == 1
    ...
    BOOLEAN                 is_paging;  /* 1, if paging is in progess */
    BOOLEAN                 is_inquiry; /* 1, if inquiry is in progess */
    fixed_queue_t           *page_queue;
    BOOLEAN                 paging;
    BOOLEAN                 discing;
    fixed_queue_t           *sec_pending_q;  /* pending sequrity requests in tBTM_SEC_QUEUE_ENTRY format */
#if  (!defined(BT_TRACE_VERBOSE) || (BT_TRACE_VERBOSE == 0))
