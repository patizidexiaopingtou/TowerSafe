    ...
    if (!close_pending) {
        ...
        if ((p_cb->max_sess > 1) &&
            ...
            if (used < p_cb->max_sess &&
                ...
                if ((RFCOMM_CreateConnection(p_cb->sec_id, p_cb->scn, 1,
...
**             In general close_conn should be set to 1 to remove registering with
**             dm pm!
**
    ...
    for (i = 0; i < BTA_JV_PM_MAX_NUM; i++) {
        ...
        if ((bta_jv_cb.pm_cb[i].state != BTA_JV_PM_FREE_ST) &&
            ...
            if (bd_counter <= 1 || (appid_counter <= 1)) {
                bta_jv_clear_pm_cb(&bta_jv_cb.pm_cb[i], 1);
            } else {
                bta_jv_clear_pm_cb(&bta_jv_cb.pm_cb[i], 0);
            }
    ...
    if (L2C_IS_VALID_PSM(psm)) {
        if (psm < 0x1001) {
            ...
            case TCS_PSM_CORDLESS:  /* 7 */
                if ( 0 == bta_sys_is_register(BTA_ID_CT) &&
                        0 == bta_sys_is_register(BTA_ID_CG) ) {
                    ret = 1;
                }
                break;

            case BT_PSM_BNEP:       /* F */
                if (0 == bta_sys_is_register(BTA_ID_PAN)) {
                    ret = 1;
                }
                break;
            ...
            case HID_PSM_INTERRUPT: /* 0x13 */
                ...
                if ( 0 == bta_sys_is_register(BTA_ID_HD) ||
                        0 == bta_sys_is_register(BTA_ID_HH) ) {
                    ret = 1;
                }
                break;
            ...
            case AVCT_PSM:          /* 0x17 */
            case AVDT_PSM:          /* 0x19 */
                if ((0 == bta_sys_is_register(BTA_ID_AV)) &&
                        (0 == bta_sys_is_register(BTA_ID_AVK))) {
                    ret = 1;
                }
                break;
...
void bta_jv_get_channel_id(tBTA_JV_MSG *p_data)
{
    ...
    case BTA_JV_CONN_TYPE_RFCOMM: {
        ...
        UINT8 scn = 0;
        if (channel > 0) {
            if (BTM_TryAllocateSCN(channel) == 0) {
...
void bta_jv_free_scn(tBTA_JV_MSG *p_data)
{
    ...
    if (fc->user_data)
    {
        ...
        if (user_data->server_status == BTA_JV_SERVER_RUNNING && find_rfc_pcb((void *)user_data->slot_id, &p_cb, &p_pcb)) {
            /* if call bta_jv_rfcomm_stop_server successfully, find_rfc_pcb shall return 0 */
 ...
 * @param[in]   with_obex:  if 1, then an additional OBEX protocol UUID will be included
...
static int add_spp_sdp(const char *name, const int channel) {
    ...
    if (!create_base_record(handle, name, channel, 0 /* with_obex */)){
...
static void bta_jv_l2cap_client_cback(UINT16 gap_handle, UINT16 event)
{
    ...
    case GAP_EVT_CONN_UNCONGESTED:
        p_cb->cong = (event == GAP_EVT_CONN_CONGESTED) ? 1 : 0;
...
void bta_jv_l2cap_start_server(tBTA_JV_MSG *p_data)
{
    ...
    */
    ...
    sec_id = bta_jv_alloc_sec_id();
    if (0 == sec_id || (0 == bta_jv_check_psm(ls->local_psm)) ||
...
static void bta_jv_port_mgmt_cl_cback(UINT32 code, UINT16 port_handle)
{
    ...
    if (code == PORT_SUCCESS) {
        ...
        evt_data.rfc_close.async = 1;
        if (p_pcb->state == BTA_JV_ST_CL_CLOSING) {
            evt_data.rfc_close.async = 0;
...
static void bta_jv_port_event_cl_cback(UINT32 code, UINT16 port_handle)
{
    ...
    if (code & PORT_EV_FC) {
        p_pcb->cong = (code & PORT_EV_FCS) ? 0 : 1;
...
void bta_jv_rfcomm_connect(tBTA_JV_MSG *p_data)
{
    ...
    */
    ...
    evt_data.status = BTA_JV_SUCCESS;
    if (0 == sec_id ||
            BTM_SetSecurityLevel(1, "", sec_id,  cc->sec_mask, BT_PSM_RFCOMM,
                                 BTM_SEC_PROTO_RFCOMM, cc->remote_scn) == 0) {
        evt_data.status = BTA_JV_FAILURE;
        APPL_TRACE_ERROR("sec_id:%d is zero or BTM_SetSecurityLevel failed, remote_scn:%d", sec_id, cc->remote_scn);
    }

    if (evt_data.status == BTA_JV_SUCCESS &&
            RFCOMM_CreateConnection(UUID_SERVCLASS_SERIAL_PORT, cc->remote_scn, 0,
    ...
    if (evt_data.status == BTA_JV_SUCCESS) {
        ...
        if (p_cb) {
            ...
            /* coverity[uninit_use_in_call]
               0-POSITIVE: port_state is initialized at PORT_GetState() */
...
void bta_jv_rfcomm_start_server(tBTA_JV_MSG *p_data)
{
    ...
    */
    ...
    do {
        ...
        if (0 == sec_id ||
                BTM_SetSecurityLevel(0, "JV PORT", sec_id,  rs->sec_mask,
                                     BT_PSM_RFCOMM, BTM_SEC_PROTO_RFCOMM, rs->local_scn) == 0) {
            APPL_TRACE_ERROR("bta_jv_rfcomm_start_server, run out of sec_id");
            break;
        }

        if (RFCOMM_CreateConnection(sec_id, rs->local_scn, 1,
...
void bta_jv_rfcomm_flow_control(tBTA_JV_MSG *p_data)
{
    ...
    PORT_FlowControl_GiveCredit(p_pcb->port_handle, 1, fc->credits_given);
}
...
{
    struct fc_channel *fc = fcchan_get(chan, 1);
    struct fc_client *t;
    uint8_t sec_id;
...
{
    struct fc_client *t = fc_clients;
    struct fc_channel *tc = fcchan_get(fc->chan, 0);
...
void bta_jv_l2cap_connect_le(tBTA_JV_MSG *p_data)
{
    ...
    t = fcclient_alloc(cc->remote_chan, 0, NULL);
    if (!t) {
...
void bta_jv_l2cap_stop_server_le(tBTA_JV_MSG *p_data)
{
    ...
    evt.l2c_close.async = 0;
    evt.l2c_close.handle = GAP_INVALID_HANDLE;

    fcchan = fcchan_get(ls->local_chan, 0);
    if (fcchan) {
...
void bta_jv_l2cap_start_server_le(tBTA_JV_MSG *p_data)
{
    ...
    t = fcclient_alloc(ss->local_chan, 1, NULL);
    if (!t) {
        goto out;
    }
...
#endif  ///defined BTA_JV_INCLUDED && BTA_JV_INCLUDED == 1
