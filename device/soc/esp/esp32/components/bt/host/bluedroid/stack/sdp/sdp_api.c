...
** Returns          BOOLEAN
**                          1 if successful
**                          0 if one or more parameters are bad
**
*******************************************************************************/
...
**
** Returns          1 if discovery cancelled, 0 if a matching activity is not found.
**
*******************************************************************************/
...
**
** Returns          1 if discovery started, 0 if failed.
**
*******************************************************************************/
    ...
    while (p_attr) {
        ...
        if ((p_attr->attr_id == ATTR_ID_ADDITION_PROTO_DESC_LISTS)
            ...
            for (p_sattr = p_attr->attr_value.v.p_sub_attr; p_sattr; p_sattr = p_sattr->p_next_attr) {
                /* Safety check - each entry should itself be a sequence */
                if (SDP_DISC_ATTR_TYPE(p_sattr->attr_len_type) == DATA_ELE_SEQ_DESC_TYPE) {
                    if ( (ret = sdp_fill_proto_elem(p_sattr, layer_uuid, p_elem)) == 1) {
                        break;
                    }
                }
            }
                    ...
                    if ((SDP_DISC_ATTR_TYPE(p_sattr->attr_len_type) == UUID_DESC_TYPE)
                        ...
                        if ((SDP_DISC_ATTR_TYPE(p_sattr->attr_len_type) == UINT_DESC_TYPE) &&
                            ...
                            *p_version = p_sattr->attr_value.v.u16;
                            ...
                            return (0);    /* The type and/or size was not valid for the profile list version */
                        }
                    }
                }
            }
...
UINT16 SDP_SetLocalDiRecord( tSDP_DI_RECORD *p_device_info, UINT32 *p_handle )
{
    ...
    *p_handle = 0;
    ...
    if ( p_device_info->primary_record == 1 && sdp_cb.server_db.di_primary_handle ) {
    ...
    *p_handle = handle;
    ...
    if ((SDP_AddServiceClassIdList(handle, 1, &di_uuid)) == 0) {
    ...
    } else if (p_device_info->primary_record == 1) {
        sdp_cb.server_db.di_primary_handle = handle;
    }

    return result;
#else   /* SDP_SERVER_ENABLED is 0 */
