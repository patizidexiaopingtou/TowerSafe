...
#if defined(GATTC_INCLUDED) && (GATTC_INCLUDED == 1)
//#if( defined GATTC_CACHE_NVS ) && (GATTC_CACHE_NVS == 1)
...
#if (defined BTA_GATT_DEBUG && BTA_GATT_DEBUG == 1)
...
static void bta_gattc_explore_srvc(UINT16 conn_id, tBTA_GATTC_SERV *p_srvc_cb)
{
    ...
    if (p_clcb->transport == BTA_TRANSPORT_LE) {
        L2CA_EnableUpdateBleConnParams(p_clcb->p_srcb->server_bda, 1);
    }
    #endif
...
**
** Returns          0 if map can not be found.
**
*******************************************************************************/
void bta_gattc_search_service(tBTA_GATTC_CLCB *p_clcb, tBT_UUID *p_uuid)
{
    ...
    for (list_node_t *sn = list_begin(p_clcb->p_srcb->p_srvc_cache);
        ...
        if (!bta_gattc_uuid_compare(p_uuid, &p_cache->uuid, 0)) {
            continue;
        }
                                     ...
                                     UINT16 *count)
{
    ...
    for (list_node_t *sn = list_begin(svc);
        ...
        if (svc_uuid == NULL || bta_gattc_uuid_compare(svc_uuid, &p_cur_srvc->uuid, 1)) {
    ...
    for (list_node_t *sn = list_begin(p_srcb->p_srvc_cache);
        ...
        if (op == GATT_OP_GET_INCLUDE_SVC) {
            ...
            for (list_node_t *isn = list_begin(p_cur_srvc->included_svc);
                ...
                if (!incl_uuid || bta_gattc_uuid_compare(&p_isvc->uuid, incl_uuid, 1)) {
                    bta_gattc_fill_gatt_db_el(curr_db_attr,
        ...
        for (list_node_t *cn = list_begin(p_cur_srvc->characteristics);
            ...
            if ((op == GATT_OP_GET_ALL_CHAR || op == GATT_OP_GET_CHAR_BY_UUID) &&
                (char_uuid == NULL || bta_gattc_uuid_compare(&p_char->uuid, char_uuid, 1))) {
            ...
            if ((op == GATT_OP_GET_DESCRI_BY_UUID) &&
                !bta_gattc_uuid_compare(&p_char->uuid, char_uuid, 1)) {
                continue;
            }
            ...
            if (op == GATT_OP_GET_ALL_DESCRI || op == GATT_OP_GET_DESCRI_BY_UUID || op == GATT_OP_GET_DESCRI_BY_HANDLE) {
                for (list_node_t *dn = list_begin(p_char->descriptors);
                    ...
                    if (((op == GATT_OP_GET_ALL_DESCRI || op == GATT_OP_GET_DESCRI_BY_UUID) &&
                        (descr_uuid == NULL || bta_gattc_uuid_compare(&p_desc->uuid, descr_uuid, 1))) ||
                        (op == GATT_OP_GET_DESCRI_BY_HANDLE && bta_gattc_uuid_compare(&p_desc->uuid, descr_uuid, 1))) {
                        bta_gattc_fill_gatt_db_el(curr_db_attr,
    ...
    for (list_node_t *sn = list_begin(services);
        ...
        if (char_uuid != NULL) {
            for (list_node_t *cn = list_begin(p_cur_srvc->characteristics);
                ...
                if ((type == BTGATT_DB_CHARACTERISTIC) && bta_gattc_uuid_compare(&p_char->uuid, char_uuid, 1)) {
                    db_size++;
                    continue;
                }

                if (p_char->descriptors && (type == BTGATT_DB_DESCRIPTOR) && bta_gattc_uuid_compare(&p_char->uuid, char_uuid, 1)) {
                    for (list_node_t *dn = list_begin(p_char->descriptors);
                         dn != list_end(p_char->descriptors); dn = list_next(dn)) {
                        tBTA_GATTC_DESCRIPTOR *p_desc = list_node(dn);
...
bool bta_gattc_cache_load(tBTA_GATTC_CLCB *p_clcb)
{
    ...
    if ((status = bta_gattc_co_cache_open(p_clcb->p_srcb->server_bda, 1, &index)) != BTA_GATT_OK) {
