        ...
        void *p_cplt_cback)
{
    ...
    case HCI_READ_TRANSMIT_POWER_LEVEL:
        btm_read_tx_power_complete(p, 0);
        break;
    ...
    case HCI_BLE_READ_ADV_CHNL_TX_POWER:
        btm_read_tx_power_complete(p, 1);
        break;
    ...
    default:
        ...
        if (status != HCI_SUCCESS) {
            ...
            case HCI_SET_CONN_ENCRYPTION:
                ...
                btm_sec_encrypt_change (BTM_INVALID_HCI_HANDLE, status, 0);
#endif  ///SMP_INCLUDED == 1
                break;
...
static void btu_hcif_num_compl_data_pkts_evt (UINT8 *p)
{
    ...
    /* Send on to SCO */
#if (BTM_SCO_HCI_INCLUDED == 1) && (BTM_SCO_INCLUDED == 1)
...
static void btu_hcif_mode_change_evt (UINT8 *p)
{
    ...
    /*
    #if (HID_DEV_INCLUDED == 1) && (HID_DEV_PM_INCLUDED == 1)
...
static void btu_ble_ll_conn_complete_evt ( UINT8 *p, UINT16 evt_len)
{
    btm_ble_conn_complete(p, evt_len, 0);
}
//#if (defined BLE_PRIVACY_SPT && BLE_PRIVACY_SPT == 1)
static void btu_ble_proc_enhanced_conn_cmpl( UINT8 *p, UINT16 evt_len)
{
    btm_ble_conn_complete(p, evt_len, 1);
}
//#endif
static void btu_ble_ll_conn_param_upd_evt (UINT8 *p, UINT16 evt_len)
{
...
static void btu_ble_ext_adv_report_evt(UINT8 *p, UINT16 evt_len)
{
    ...
    while (num_reports--) {
        ...
        if(addr_type != 0xFF) {
#if (defined BLE_PRIVACY_SPT && BLE_PRIVACY_SPT == 1)
            btm_identity_addr_to_random_pseudo(bda, &addr_type, 0);
#endif
        }
