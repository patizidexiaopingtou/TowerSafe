...
void btm_acl_init (void)
{
    ...
    memset (btm_cb.btm_scn, 0, BTM_MAX_SCN);          /* Initialize the SCN usage to 0 */
...
**                  connection.
**
** Returns          success return 1, otherwise 0.
**
*******************************************************************************/
    ...
    /* some devices do not support switch while encryption is on */
        ...
        if ((p_dev_rec != NULL)
            ...
            if (p->encrypt_state != BTM_ACL_ENCRYPT_STATE_ENCRYPT_OFF) {
                if (!btsnd_hcic_set_conn_encrypt (p->hci_handle, 0)) {
                    return (BTM_NO_RESOURCES);
                } else {
...
void btm_acl_role_changed (UINT8 hci_status, BD_ADDR bd_addr, UINT8 new_role)
{
    ...
    if (p->switch_role_state == BTM_ACL_SWKEY_STATE_SWITCHING) {
        if (btsnd_hcic_set_conn_encrypt (p->hci_handle, 1)) {
...
BOOLEAN BTM_TryAllocateSCN(UINT8 scn)
{
    ...
    if (!btm_cb.btm_scn[scn - 1]) {
        btm_cb.btm_scn[scn - 1] = 1;
        return 1;
    }

    return (0);     /* Port was busy */
}
...
BOOLEAN BTM_FreeSCN(UINT8 scn)
{
    ...
    if (scn <= BTM_MAX_SCN) {
        btm_cb.btm_scn[scn - 1] = 0;
        return (1);
    } else {
        return (0);    /* Illegal SCN passed in */
    }
    return (0);
}
#endif  ///CLASSIC_BT_INCLUDED == 1
...
**
** Description      set paging to 0 and free the page queue - called at hci_reset
**
*******************************************************************************/
...
**
** Returns          1 if sent out to upper layer,
**                  0 if no one needs the notification.
**
*******************************************************************************/
