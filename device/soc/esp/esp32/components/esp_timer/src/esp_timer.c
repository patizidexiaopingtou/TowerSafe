...
static int s_timer_task = -1;
static EVENT_CB_S s_timer_event = {0};
...
{
	s_timer_task = LOS_CurTaskIDGet();
    while (true){
        // ulTaskNotifyTake(pdTRUE, portMAX_DELAY);
        // LOS_TaskSuspend(s_timer_task);
        LOS_EventRead(&s_timer_event,0x01,LOS_WAITMODE_AND | LOS_WAITMODE_CLR, LOS_WAIT_FOREVER);
...
{
	if(-1 != s_timer_task) {
		// if(s_timer_task != LOS_CurTaskIDGet()) LOS_TaskResume(s_timer_task);
		LOS_EventWrite(&s_timer_event,0x01);
    	portYIELD_FROM_ISR();
	}
}
...
esp_err_t esp_timer_init(void)
{
...
#else
	{
		...
		attr.pfnTaskEntry = (TSK_ENTRY_FUNC)timer_task;
		attr.usTaskPrio = configMAX_PRIORITIES - 1 - ESP_TASK_TIMER_PRIO;
		attr.uwArg = 0;
		attr.uwStackSize = ESP_TASK_TIMER_STACK + 32;
		attr.pcName = "esp_timer";
		memset(&s_timer_event,0,sizeof(s_timer_event));
		LOS_EventInit(&s_timer_event);
		ret = LOS_TaskCreate((UINT32 *)&s_timer_task, &attr);
	    if (ret != LOS_OK) {
	        err = ESP_ERR_NO_MEM;
	        goto out;
	    }
...
out:
	ESP_LOGE(TAG, "esp_timer_init fail!!!");
    if (-1 != s_timer_task) {
        LOS_TaskDelete(s_timer_task);
        s_timer_task = -1;
		LOS_EventDestroy(&s_timer_event);
    }
